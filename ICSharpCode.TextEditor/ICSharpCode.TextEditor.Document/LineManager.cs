...
internal sealed class LineManager
{
	...
	private void RemoveInternal(ref DeferredEventList deferredEventList, int offset, int length)
	{
		...
		int num2 = byOffset.Offset + byOffset.TotalLength - (offset + length);
	...
	public int GetVisibleLine(int logicalLineNumber)
	{
		...
		int num = 0;
		int num2 = 0;
		foreach (FoldMarker topLevelFoldedFolding in document.FoldingManager.GetTopLevelFoldedFoldings())
		{
			if (topLevelFoldedFolding.StartLine >= logicalLineNumber)
			{
				break;
			}
			if (topLevelFoldedFolding.StartLine >= num2)
			{
				num += topLevelFoldedFolding.StartLine - num2;
				if (topLevelFoldedFolding.EndLine > logicalLineNumber)
				{
					return num;
				}
				num2 = topLevelFoldedFolding.EndLine;
			}
		}
