...
public class DefaultHighlightingStrategy : IHighlightingStrategyUsingRuleSets, IHighlightingStrategy
{
	...
	public DefaultHighlightingStrategy(string name)
	{
		...
		environmentColors["FoldLine"] = new HighlightColor(Color.FromArgb(128, 128, 128), Color.Black, bold: false, italic: false);
		environmentColors["FoldMarker"] = new HighlightColor(Color.FromArgb(128, 128, 128), Color.White, bold: false, italic: false);
		environmentColors["SelectedFoldLine"] = new HighlightColor(Color.Black, bold: false, italic: false);
	...
	private bool MarkTokensInLine(IDocument document, int lineNumber, ref bool spanChanged)
	{
		...
		if (currentLine.HighlightSpanStack != currentSpanStack)
		{
			...
			{
				SpanStack.Enumerator enumerator2 = currentSpanStack.GetEnumerator();
				SpanStack.Enumerator enumerator3 = currentLine.HighlightSpanStack.GetEnumerator();
				bool flag = false;
				while (!flag)
				{
					bool flag2 = false;
					...
					if (flag2 | flag3)
					{
						if (flag2 & flag3)
						{
							if (enumerator2.Current != enumerator3.Current)
							{
								flag = true;
								result = true;
								spanChanged = true;
	...
	private static bool MatchExpr(LineSegment lineSegment, char[] expr, int index, IDocument document, bool ignoreCase)
	{
		...
		while (num < expr.Length)
		{
			...
			if (c == '@')
			{
				...
				if (num < expr.Length)
				{
					...
					case '!':
					{
						...
						{
							char num3 = (ignoreCase ? char.ToUpperInvariant(document.GetCharAt(lineSegment.Offset + index + num2 + i)) : document.GetCharAt(lineSegment.Offset + index + num2 + i));
							char c2 = (ignoreCase ? char.ToUpperInvariant(stringBuilder[i]) : stringBuilder[i]);
							if (num3 != c2)
							{
								break;
							}
					...
					case '-':
					{
						...
						{
							char num4 = (ignoreCase ? char.ToUpperInvariant(document.GetCharAt(lineSegment.Offset + index - stringBuilder2.Length + j)) : document.GetCharAt(lineSegment.Offset + index - stringBuilder2.Length + j));
							char c3 = (ignoreCase ? char.ToUpperInvariant(stringBuilder2[j]) : stringBuilder2[j]);
							if (num4 != c3)
							{
								break;
							}
			...
			else
			{
				...
				char num5 = (ignoreCase ? char.ToUpperInvariant(document.GetCharAt(lineSegment.Offset + index + num2)) : document.GetCharAt(lineSegment.Offset + index + num2));
				char c4 = (ignoreCase ? char.ToUpperInvariant(expr[num]) : expr[num]);
				if (num5 != c4)
				{
					return false;
				}
