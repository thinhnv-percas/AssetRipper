	...
	protected override void Convert(IDocument document, int y1, int y2)
	{
		for (int num = y2; num >= y1; num--)
		{
			...
			if (lineSegment.Length > 0)
			{
				...
				if (num2 > 0)
				{
					string text = document.GetText(lineSegment.Offset, num2).Replace("\t", new string(' ', document.TextEditorProperties.TabIndent));
					document.Replace(lineSegment.Offset, num2, text);
				}
			}
		}
	}
