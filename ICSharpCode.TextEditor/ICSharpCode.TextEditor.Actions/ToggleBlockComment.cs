...
public class ToggleBlockComment : AbstractEditAction
{
	...
	public static BlockCommentRegion FindSelectedCommentRegion(IDocument document, string commentStart, string commentEnd, int selectionStartOffset, int selectionEndOffset)
	{
		...
		num = ((num2 < 0) ? text.IndexOf(commentEnd) : text.IndexOf(commentEnd, num2 + commentStart.Length - selectionStartOffset));
		if (num >= 0)
		{
		...
		if (num2 == -1)
		{
			int num3 = selectionEndOffset + commentStart.Length - 1;
			if (num3 > document.TextLength)
			{
				num3 = document.TextLength;
			}
			string text2 = document.GetText(0, num3);
			num2 = text2.LastIndexOf(commentStart);
			if (num2 >= 0 && text2.IndexOf(commentEnd, num2, selectionStartOffset - num2) > num2)
			{
				num2 = -1;
			}
		}
		if (num == -1)
		{
			int num4 = selectionStartOffset + 1 - commentEnd.Length;
			if (num4 < 0)
			{
				num4 = selectionStartOffset;
			}
			num = document.GetText(num4, document.TextLength - num4).IndexOf(commentEnd);
			if (num >= 0)
			{
				num += num4;
			}
