...
public static class PatternMatching
{
	...
	{
		if (node is ILExpression { Prefixes: null } iLExpression)
		{
	...
	{
		if (node is ILExpression { Prefixes: null } iLExpression && iLExpression.Code == code && iLExpression.Arguments.Count == 0)
		{
	...
	{
		if (node is ILExpression { Prefixes: null } iLExpression && iLExpression.Code == code)
		{
	...
	public static bool MatchThis(this ILNode node)
	{
		...
		{
			return operand.OriginalParameter.IsHiddenThisParameter;
		}
		return false;
	}
