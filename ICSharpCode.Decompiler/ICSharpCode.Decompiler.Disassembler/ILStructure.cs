...
public class ILStructure
{
		...
		: this(ILStructureType.Root, 0, body.GetCodeSize())
	{
		...
		for (int num = list.Count - 1; num >= 0; num--)
		{
			...
			int offset = (int)list[num].Value.Offset;
			if (offset < endOffset)
			{
				Instruction instruction = null;
				Instruction previous = body.GetPrevious(list[num].Value);
	...
	private List<KeyValuePair<Instruction, Instruction>> FindAllBranches(CilBody body)
	{
		...
		foreach (Instruction instruction in body.Instructions)
		{
			...
			case OperandType.ShortInlineBrTarget:
				if (instruction.Operand is Instruction value)
				{
					list.Add(new KeyValuePair<Instruction, Instruction>(instruction, value));
				}
				break;
			case OperandType.InlineSwitch:
				if (!(instruction.Operand is IList<Instruction> list2))
				{
					break;
				}
				foreach (Instruction item in list2)
				{
					if (item != null)
					{
						list.Add(new KeyValuePair<Instruction, Instruction>(instruction, item));
					}
				}
				break;
			}
