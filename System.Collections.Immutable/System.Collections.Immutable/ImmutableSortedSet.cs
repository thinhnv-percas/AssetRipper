...
public sealed class ImmutableSortedSet<T> : IImmutableSet<T>, IReadOnlyCollection<T>, IEnumerable<T>, IEnumerable, ISortKeyCollection<T>, IReadOnlyList<T>, IList<T>, ICollection<T>, ISet<T>, IList, ICollection, IStrongEnumerable<T, ImmutableSortedSet<T>.Enumerator>
{
	...
	public struct Enumerator : IEnumerator<T>, IEnumerator, IDisposable, ISecurePooledObjectUser, IStrongEnumerator<T>
	{
		...
		internal Enumerator(Node root, Builder builder = null, bool reverse = false)
		{
			...
			{
				_stack = s_enumeratingStacks.PrepNew(this, (Stack<RefAsValueType<Node>>)(object)new Stack<RefAsValueType<ImmutableSortedSet<RefAsValueType<Node>>.Node>>(root.Height));
			}
			PushNext(_root);
		}
		...
		public bool MoveNext()
		{
			...
			if (((Stack<RefAsValueType<ImmutableSortedSet<RefAsValueType<Node>>.Node>>)(object)val).Count > 0)
			{
				Node node = (_current = ((Stack<RefAsValueType<ImmutableSortedSet<RefAsValueType<Node>>.Node>>)(object)val).Pop().Value);
		...
		private void PushNext(Node node)
		{
			...
			while (!node.IsEmpty)
			{
				((Stack<RefAsValueType<ImmutableSortedSet<RefAsValueType<Node>>.Node>>)(object)val).Push((RefAsValueType<ImmutableSortedSet<RefAsValueType<Node>>.Node>)new RefAsValueType<Node>(node));
	...
	public bool SetEquals(IEnumerable<T> other)
	{
		...
		SortedSet<T> val = new SortedSet<T>(other, KeyComparer);
		if (Count != val.Count)
		{
			return false;
		}
		int num = 0;
		Enumerator<T> enumerator = val.GetEnumerator();
		try
		{
			while (enumerator.MoveNext())
			{
