...
public sealed class ImmutableList<T> : IImmutableList<T>, IReadOnlyList<T>, IEnumerable<T>, IEnumerable, IReadOnlyCollection<T>, IList<T>, ICollection<T>, IList, ICollection, IOrderedCollection<T>, IImmutableListQueries<T>, IStrongEnumerable<T, ImmutableList<T>.Enumerator>
{
	...
	public struct Enumerator : IEnumerator<T>, IEnumerator, IDisposable, ISecurePooledObjectUser, IStrongEnumerator<T>
	{
		...
		internal Enumerator(Node root, Builder builder = null, int startIndex = -1, int count = -1, bool reversed = false)
		{
			...
			if (_count > 0)
			{
				...
				{
					_stack = s_EnumeratingStacks.PrepNew(this, (Stack<RefAsValueType<Node>>)(object)new Stack<RefAsValueType<ImmutableList<RefAsValueType<Node>>.Node>>(root.Height));
				}
				ResetStack();
			}
		}
		...
		public bool MoveNext()
		{
			...
			if (_stack != null)
			{
				...
				if (_remainingCount > 0 && ((Stack<RefAsValueType<ImmutableList<RefAsValueType<Node>>.Node>>)(object)val).Count > 0)
				{
					PushNext(NextBranch(_current = ((Stack<RefAsValueType<ImmutableList<RefAsValueType<Node>>.Node>>)(object)val).Pop().Value));
					_remainingCount--;
					return true;
				}
			}
		...
		{
			Stack<RefAsValueType<Node>> val = _stack.Use(ref this);
			val.ClearFastWhenEmpty<RefAsValueType<Node>>();
			Node node = _root;
			...
			while (!node.IsEmpty && num != PreviousBranch(node).Count)
			{
				...
				{
					((Stack<RefAsValueType<ImmutableList<RefAsValueType<Node>>.Node>>)(object)val).Push((RefAsValueType<ImmutableList<RefAsValueType<Node>>.Node>)new RefAsValueType<Node>(node));
	...
	internal sealed class Node : IBinaryTree<T>, IBinaryTree, IEnumerable<T>, IEnumerable
	{
		...
		internal ImmutableList<TOutput>.Node ConvertAll<TOutput>(Func<T, TOutput> converter)
		{
			...
			return emptyNode.AddRange(Enumerable.Select<T, TOutput>((IEnumerable<T>)this, converter));
		}
