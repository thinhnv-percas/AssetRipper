...
public static class ImmutableSortedDictionary
{
	...
	public static ImmutableSortedDictionary<TKey, TValue> ToImmutableSortedDictionary<TSource, TKey, TValue>(this IEnumerable<TSource> source, Func<TSource, TKey> keySelector, Func<TSource, TValue> elementSelector, IComparer<TKey> keyComparer, IEqualityComparer<TValue> valueComparer)
	{
		...
		return ImmutableSortedDictionary<TKey, TValue>.Empty.WithComparers(keyComparer, valueComparer).AddRange(Enumerable.Select<TSource, KeyValuePair<TKey, TValue>>(source, (Func<TSource, KeyValuePair<TKey, TValue>>)((TSource element) => new KeyValuePair<TKey, TValue>(keySelector(element), elementSelector(element)))));
	}
...
public sealed class ImmutableSortedDictionary<TKey, TValue> : IImmutableDictionary<TKey, TValue>, IReadOnlyDictionary<TKey, TValue>, IEnumerable<KeyValuePair<TKey, TValue>>, IEnumerable, IReadOnlyCollection<KeyValuePair<TKey, TValue>>, ISortKeyCollection<TKey>, IDictionary<TKey, TValue>, ICollection<KeyValuePair<TKey, TValue>>, IDictionary, ICollection
{
	...
	public struct Enumerator : IEnumerator<KeyValuePair<TKey, TValue>>, IEnumerator, IDisposable, ISecurePooledObjectUser
	{
		...
		internal Enumerator(Node root, Builder builder = null)
		{
			...
			if (!_root.IsEmpty)
			{
				...
				{
					_stack = s_enumeratingStacks.PrepNew(this, (Stack<RefAsValueType<Node>>)(object)new Stack<RefAsValueType<ImmutableSortedDictionary<RefAsValueType<Node>, _003F>.Node>>(root.Height));
				}
				PushLeft(_root);
			}
		}
		...
		public void Dispose()
		{
			...
			{
				value.ClearFastWhenEmpty<RefAsValueType<Node>>();
		...
		public bool MoveNext()
		{
			...
			if (_stack != null)
			{
				Stack<RefAsValueType<Node>> val = _stack.Use(ref this);
				if (((Stack<RefAsValueType<ImmutableSortedDictionary<RefAsValueType<Node>, _003F>.Node>>)(object)val).Count > 0)
				{
					PushLeft((_current = ((Stack<RefAsValueType<ImmutableSortedDictionary<RefAsValueType<Node>, _003F>.Node>>)(object)val).Pop().Value).Right);
					return true;
				}
			}
			_current = null;
		...
		public void Reset()
		{
			...
			if (_stack != null)
			{
				...
				stack.ClearFastWhenEmpty<RefAsValueType<Node>>();
				PushLeft(_root);
			}
		}
		...
		private void PushLeft(Node node)
		{
			...
			while (!node.IsEmpty)
			{
				((Stack<RefAsValueType<ImmutableSortedDictionary<RefAsValueType<Node>, _003F>.Node>>)(object)val).Push((RefAsValueType<ImmutableSortedDictionary<RefAsValueType<Node>, _003F>.Node>)new RefAsValueType<Node>(node));
				node = node.Left;
			}
		}
	}
	...
	internal sealed class Node : IBinaryTree<KeyValuePair<TKey, TValue>>, IBinaryTree, IEnumerable<KeyValuePair<TKey, TValue>>, IEnumerable
	{
		...
		internal IEnumerable<TKey> Keys => Enumerable.Select<KeyValuePair<TKey, TValue>, TKey>((IEnumerable<KeyValuePair<TKey, TValue>>)this, (Func<KeyValuePair<TKey, TValue>, TKey>)((KeyValuePair<TKey, TValue> p) => p.Key));

		internal IEnumerable<TValue> Values => Enumerable.Select<KeyValuePair<TKey, TValue>, TValue>((IEnumerable<KeyValuePair<TKey, TValue>>)this, (Func<KeyValuePair<TKey, TValue>, TValue>)((KeyValuePair<TKey, TValue> p) => p.Value));
		...
		internal static Node NodeTreeFromSortedDictionary(SortedDictionary<TKey, TValue> dictionary)
		{
			...
			IOrderedCollection<KeyValuePair<TKey, TValue>> orderedCollection = ((IEnumerable<KeyValuePair<TKey, TValue>>)dictionary).AsOrderedCollection();
		...
		private Node SetOrAdd(TKey key, TValue value, IComparer<TKey> keyComparer, IEqualityComparer<TValue> valueComparer, bool overwriteExistingValue, out bool replacedExistingValue, out bool mutated)
		{
			...
			else
			{
				...
				{
					throw new ArgumentException(string.Format(CultureInfo.CurrentCulture, System.SR.DuplicateKey, key));
				}
				mutated = true;
	...
	private ImmutableSortedDictionary<TKey, TValue> FillFromEmpty(IEnumerable<KeyValuePair<TKey, TValue>> items, bool overwriteOnCollision)
	{
		...
		SortedDictionary<TKey, TValue> val;
		if (items is IDictionary<TKey, TValue> dictionary)
		{
			val = new SortedDictionary<TKey, TValue>(dictionary, KeyComparer);
		}
		else
		{
			val = new SortedDictionary<TKey, TValue>(KeyComparer);
			TValue x = default(TValue);
			foreach (KeyValuePair<TKey, TValue> item in items)
			{
				...
				else if (val.TryGetValue(item.Key, ref x))
				{
					if (!_valueComparer.Equals(x, item.Value))
					{
						throw new ArgumentException(string.Format(CultureInfo.CurrentCulture, System.SR.DuplicateKey, item.Key));
					}
				}
				else
				{
		...
		Node root = Node.NodeTreeFromSortedDictionary(val);
		return new ImmutableSortedDictionary<TKey, TValue>(root, val.Count, KeyComparer, ValueComparer);
	}
}
