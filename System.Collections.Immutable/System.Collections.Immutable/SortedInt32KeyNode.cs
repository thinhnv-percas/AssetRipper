...
internal sealed class SortedInt32KeyNode<TValue> : IBinaryTree
{
	...
	public struct Enumerator : IEnumerator<KeyValuePair<int, TValue>>, IEnumerator, IDisposable, ISecurePooledObjectUser
	{
		...
		internal Enumerator(SortedInt32KeyNode<TValue> root)
		{
			...
			if (!_root.IsEmpty)
			{
				...
				{
					_stack = s_enumeratingStacks.PrepNew(this, (Stack<RefAsValueType<SortedInt32KeyNode<TValue>>>)(object)new Stack<RefAsValueType<SortedInt32KeyNode<RefAsValueType<SortedInt32KeyNode<TValue>>>>>(root.Height));
				}
				PushLeft(_root);
			}
		}
		...
		public void Dispose()
		{
			...
			{
				value.ClearFastWhenEmpty<RefAsValueType<SortedInt32KeyNode<TValue>>>();
		...
		public bool MoveNext()
		{
			...
			if (_stack != null)
			{
				Stack<RefAsValueType<SortedInt32KeyNode<TValue>>> val = _stack.Use(ref this);
				if (((Stack<RefAsValueType<SortedInt32KeyNode<RefAsValueType<SortedInt32KeyNode<TValue>>>>>)(object)val).Count > 0)
				{
					PushLeft((_current = ((Stack<RefAsValueType<SortedInt32KeyNode<RefAsValueType<SortedInt32KeyNode<TValue>>>>>)(object)val).Pop().Value).Right);
					return true;
				}
			}
			_current = null;
		...
		private void PushLeft(SortedInt32KeyNode<TValue> node)
		{
			...
			while (!node.IsEmpty)
			{
				((Stack<RefAsValueType<SortedInt32KeyNode<RefAsValueType<SortedInt32KeyNode<TValue>>>>>)(object)val).Push((RefAsValueType<SortedInt32KeyNode<RefAsValueType<SortedInt32KeyNode<TValue>>>>)new RefAsValueType<SortedInt32KeyNode<TValue>>(node));
				node = node.Left;
			}
		}
	}
	...
	{
		SortedInt32KeyNode<TValue> sortedInt32KeyNode = Search(key);
