...
public static class ImmutableDictionary
{
	...
	public static ImmutableDictionary<TKey, TValue> ToImmutableDictionary<TSource, TKey, TValue>(this IEnumerable<TSource> source, Func<TSource, TKey> keySelector, Func<TSource, TValue> elementSelector, IEqualityComparer<TKey> keyComparer, IEqualityComparer<TValue> valueComparer)
	{
		...
		return ImmutableDictionary<TKey, TValue>.Empty.WithComparers(keyComparer, valueComparer).AddRange(Enumerable.Select<TSource, KeyValuePair<TKey, TValue>>(source, (Func<TSource, KeyValuePair<TKey, TValue>>)((TSource element) => new KeyValuePair<TKey, TValue>(keySelector(element), elementSelector(element)))));
	}
...
public sealed class ImmutableDictionary<TKey, TValue> : IImmutableDictionary<TKey, TValue>, IReadOnlyDictionary<TKey, TValue>, IEnumerable<KeyValuePair<TKey, TValue>>, IEnumerable, IReadOnlyCollection<KeyValuePair<TKey, TValue>>, IImmutableDictionaryInternal<TKey, TValue>, IHashKeyCollection<TKey>, IDictionary<TKey, TValue>, ICollection<KeyValuePair<TKey, TValue>>, IDictionary, ICollection
{
	...
	public sealed class Builder : IDictionary<TKey, TValue>, ICollection<KeyValuePair<TKey, TValue>>, IEnumerable<KeyValuePair<TKey, TValue>>, IEnumerable, IReadOnlyDictionary<TKey, TValue>, IReadOnlyCollection<KeyValuePair<TKey, TValue>>, IDictionary, ICollection
	{
		...
		public IEqualityComparer<TKey> KeyComparer
		{
			...
			set
			{
				...
				if (value != KeyComparer)
				{
					...
					MutationInput origin = new MutationInput(SortedInt32KeyNode<HashBucket>.EmptyNode, comparers, 0);
		...
		private MutationInput Origin => new MutationInput(Root, _comparers, _count);
	...
	private readonly struct MutationInput
	{
		...
		internal MutationInput(SortedInt32KeyNode<HashBucket> root, Comparers comparers, int count)
		{
			_root = root;
			_comparers = comparers;
			_count = count;
	...
	private static bool ContainsKey(TKey key, MutationInput origin)
	{
		...
		{
			return value.TryGetValue(key, origin.KeyOnlyComparer, out value2);
		}
		return false;
	}
	...
	private static bool Contains(KeyValuePair<TKey, TValue> keyValuePair, MutationInput origin)
	{
		...
		if (origin.Root.TryGetValue(hashCode, out var value))
		{
			if (value.TryGetValue(keyValuePair.Key, origin.KeyOnlyComparer, out var value2))
			{
	...
	private static bool TryGetValue(TKey key, MutationInput origin, out TValue value)
	{
		...
		{
			return value2.TryGetValue(key, origin.KeyOnlyComparer, out value);
		}
		value = default(TValue);
		return false;
	}
	...
	private static bool TryGetKey(TKey equalKey, MutationInput origin, out TKey actualKey)
	{
		...
		if (origin.Root.TryGetValue(hashCode, out var value))
		{
			return value.TryGetKey(equalKey, origin.KeyOnlyComparer, out actualKey);
		}
		actualKey = equalKey;
		return false;
	}
	...
	private static MutationResult Remove(TKey key, MutationInput origin)
	{
		...
		if (origin.Root.TryGetValue(hashCode, out var value))
		{
			SortedInt32KeyNode<HashBucket> root = UpdateRoot(origin.Root, hashCode, value.Remove(key, origin.KeyOnlyComparer, out var result), origin.HashBucketComparer);
			return new MutationResult(root, (result == OperationResult.SizeChanged) ? (-1) : 0);
		}
		return new MutationResult(origin);
	}
