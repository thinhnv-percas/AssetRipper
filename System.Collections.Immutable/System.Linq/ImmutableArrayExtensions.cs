...
public static class ImmutableArrayExtensions
{
	public static IEnumerable<TResult> Select<T, TResult>(this ImmutableArray<T> immutableArray, Func<T, TResult> selector)
	{
		...
		return Enumerable.Select<T, TResult>((IEnumerable<T>)immutableArray.array, selector);
	}
	...
	public static IEnumerable<TResult> SelectMany<TSource, TCollection, TResult>(this ImmutableArray<TSource> immutableArray, Func<TSource, IEnumerable<TCollection>> collectionSelector, Func<TSource, TCollection, TResult> resultSelector)
	{
		...
		{
			return Enumerable.SelectMany<TSource, TCollection, TResult>((IEnumerable<TSource>)immutableArray, collectionSelector, resultSelector);
		}
	...
	{
		immutableArray.ThrowNullRefIfNotInitialized();
		return Enumerable.Where<T>((IEnumerable<T>)immutableArray.array, predicate);
	}
	...
	public static T First<T>(this ImmutableArray<T> immutableArray, Func<T, bool> predicate)
	{
		...
		return Enumerable.First<T>(Enumerable.Empty<T>());
	}
	...
	public static T First<T>(this ImmutableArray<T> immutableArray)
	{
		if (immutableArray.Length <= 0)
		{
			return Enumerable.First<T>((IEnumerable<T>)immutableArray.array);
		}
	...
	public static T Last<T>(this ImmutableArray<T> immutableArray)
	{
		if (immutableArray.Length <= 0)
		{
			return Enumerable.Last<T>((IEnumerable<T>)immutableArray.array);
		}
	...
	public static T Last<T>(this ImmutableArray<T> immutableArray, Func<T, bool> predicate)
	{
		...
		return Enumerable.Last<T>(Enumerable.Empty<T>());
	}

	public static T LastOrDefault<T>(this ImmutableArray<T> immutableArray)
	{
		immutableArray.ThrowNullRefIfNotInitialized();
		return Enumerable.LastOrDefault<T>((IEnumerable<T>)immutableArray.array);
	}
	...
	{
		immutableArray.ThrowNullRefIfNotInitialized();
		return Enumerable.Single<T>((IEnumerable<T>)immutableArray.array);
	}
	...
	public static T Single<T>(this ImmutableArray<T> immutableArray, Func<T, bool> predicate)
	{
		...
		foreach (T val in array)
		{
			if (predicate(val))
			{
				if (!flag)
				{
					Enumerable.Single<byte>((IEnumerable<byte>)ImmutableArray.TwoElementArray);
				}
				flag = false;
				result = val;
			}
		...
		if (flag)
		{
			Enumerable.Single<T>(Enumerable.Empty<T>());
		}
		return result;
	}
	...
	{
		immutableArray.ThrowNullRefIfNotInitialized();
		return Enumerable.SingleOrDefault<T>((IEnumerable<T>)immutableArray.array);
	}
