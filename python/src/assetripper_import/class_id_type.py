"""
Port of Source/AssetRipper.SourceGenerated.Extensions.SourceGenerator/ClassIDType.cs

An identifier for each Unity object type. Upstream this enum is generated by
AssemblyDumper's Pass556 into AssetRipper.SourceGenerated; the file ported here is the
copy checked into the repo (with abstract classes removed), which makes it the one piece
of that generated assembly available to this port.

Member names deliberately keep upstream's PascalCase rather than this project's usual
SCREAMING_SNAKE enum convention: they are Unity's own class names, are compared against
type-tree type names as strings, and mechanical conversion would mangle them
