"""Port of Source/AssetRipper.Import/Platforms/WindowsGameStructure.cs"""
...
class WindowsGameStructure(PlatformGameStructure):
...
def _is_unity_data_directory(folder_path: str, file_system) -> bool:
    ...
    exe_path = file_system.path.join(root_path, game_name + _EXE_EXTENSION)
    return file_system.file.exists(exe_path)


def _is_executable_file(file_path: str, file_system) -> bool:
    return bool(file_path) and file_path.lower().endswith(_EXE_EXTENSION) and file_system.file.exists(file_path)
...
def _get_data_directory(root_directory: str, file_system) -> tuple[bool, str | None, str | None]:
    ...
    for file in file_system.directory.enumerate_files(root_directory):
        if file.lower().endswith(_EXE_EXTENSION):
            exe_count += 1
    ...
    if exe_count > 0:
        name = name if exe_count == 1 else file_system.path.get_file_name(root_directory)
