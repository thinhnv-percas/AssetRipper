"""Port of Source/AssetRipper.Export.UnityProjects/RawAssets/UnknownExportCollection.cs"""
from __future__ import annotations

from assetripper_io_files.filesystem import MAX_FILE_NAME_LENGTH

from ..i_export_collection import IExportCollection
from ..meta_ptr import NULL_PTR


class UnknownExportCollection(IExportCollection):
    ...
    def export(self, container, project_directory: str, file_system) -> bool:
        resource_path = file_system.path.join(
            project_directory, "AssetRipper", "UnknownAssets", self.asset.class_name, f"{self.asset.name}.unknown"
        )
    ...
    def get_export_id(self, container, asset) -> int:
        raise NotImplementedError("UnknownExportCollection does not support export IDs")
