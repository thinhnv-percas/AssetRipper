...
{
	private static readonly bool on_mono = Type.GetType("Mon3.Runtime") != null;
	...
	public virtual AssemblyDefinition Resolve(AssemblyNameReference name, ReaderParameters parameters)
	{
		...
		{
			assemblyDefinition = ResolveFailure(this, name);
			if (assemblyDefinition != null)
			{
				return assemblyDefinition;
			}
	...
	private static Collection<string> GetDefaultMon3GacPaths()
	{
		Collection<string> collection = new Collection<string>(1);
		string currentMon3Gac = GetCurrentMon3Gac();
		if (currentMon3Gac != null)
		{
			collection.Add(currentMon3Gac);
		}
		...
		foreach (string text in array)
		{
			if (!string.IsNullOrEmpty(text))
			{
				...
				if (Directory.Exists(text2) && !collection.Contains(currentMon3Gac))
				{
	...
	private static string GetCurrentMon3Gac()
	{
	...
	private AssemblyDefinition GetAssemblyInGac(AssemblyNameReference reference, ReaderParameters parameters)
	{
		...
		if (on_mono)
		{
			return GetAssemblyInMon3Gac(reference, parameters);
		}
		return GetAssemblyInNetGac(reference, parameters);
	}

	private AssemblyDefinition GetAssemblyInMon3Gac(AssemblyNameReference reference, ReaderParameters parameters)
	{
