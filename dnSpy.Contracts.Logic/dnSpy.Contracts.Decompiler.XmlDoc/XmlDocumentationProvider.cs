...
public class XmlDocumentationProvider : IDeserializationCallback
{
	...
	private sealed class LinePositionMapper
	{
		...
		public int GetPositionForLine(int line)
		{
			...
			while (line > currentLine)
			{
				...
				array[0] = (byte)num;
				decoder.Convert(array, 0, 1, array2, 0, array2.Length, flush: false, out var bytesUsed, out var charsUsed, out var _);
	...
	private Encoding GetEncoding(Encoding encoding)
	{
		...
		Debug.Assert(!encoding2.IsReadOnly);
		if (encoding2.IsReadOnly)
		{
			return encoding;
		}
		encoding2.EncoderFallback = new EncoderReplacementFallback("\ufffd");
		encoding2.DecoderFallback = new DecoderReplacementFallback("\ufffd");
		return encoding2;
	}
	...
	public XmlDocumentationProvider(string fileName)
	{
		...
		if (string.IsNullOrEmpty(xmlTextReader.GetAttribute("redirect")))
		{
			...
			encoding = GetEncoding(xmlTextReader.Encoding);
		...
		if (redirectionTarget != null)
		{
			using (FileStream input2 = new FileStream(redirectionTarget, FileMode.Open, FileAccess.Read, FileShare.Read | FileShare.Delete))
			{
				...
				encoding = GetEncoding(xmlTextReader2.Encoding);
	...
	public string GetDocumentation(StringBuilder key)
	{
		...
		return GetDocumentation(key.ToString(), allowReload: true);
	}
