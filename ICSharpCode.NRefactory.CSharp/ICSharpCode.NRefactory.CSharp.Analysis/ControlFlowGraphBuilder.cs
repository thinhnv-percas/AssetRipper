...
public class ControlFlowGraphBuilder
{
	private sealed class NodeCreationVisitor : DepthFirstAstVisitor<ControlFlowNode, ControlFlowNode>
	{
		...
		{
			ControlFlowNode controlFlowNode = HandleStatementList(blockStatement.Statements, data);
			return CreateConnectedEndNode(blockStatement, controlFlowNode);
		}
		...
		public override ControlFlowNode VisitIfElseStatement(IfElseStatement ifElseStatement, ControlFlowNode data)
		{
			...
			ControlFlowNode controlFlowNode2 = ifElseStatement.TrueStatement.AcceptVisitor(this, controlFlowNode);
			ControlFlowNode controlFlowNode3 = builder.CreateStartNode(ifElseStatement.FalseStatement);
			if (flag != true)
			{
				Connect(data, controlFlowNode3, ControlFlowEdgeType.ConditionFalse);
			}
			ControlFlowNode controlFlowNode4 = ifElseStatement.FalseStatement.AcceptVisitor(this, controlFlowNode3);
			ControlFlowNode controlFlowNode5 = builder.CreateEndNode(ifElseStatement);
			Connect(controlFlowNode2, controlFlowNode5);
			if (controlFlowNode4 != null)
			{
				Connect(controlFlowNode4, controlFlowNode5);
			}
			else if (flag != true)
			{
				Connect(data, controlFlowNode5, ControlFlowEdgeType.ConditionFalse);
			}
			return controlFlowNode5;
		}
		...
		public override ControlFlowNode VisitWhileStatement(WhileStatement whileStatement, ControlFlowNode data)
		{
			...
			ControlFlowNode controlFlowNode4 = whileStatement.EmbeddedStatement.AcceptVisitor(this, controlFlowNode3);
			Connect(controlFlowNode4, controlFlowNode2);
			if (flag != true)
			{
		...
		public override ControlFlowNode VisitDoWhileStatement(DoWhileStatement doWhileStatement, ControlFlowNode data)
		{
			...
			ControlFlowNode controlFlowNode4 = doWhileStatement.EmbeddedStatement.AcceptVisitor(this, controlFlowNode3);
			Connect(controlFlowNode4, controlFlowNode2);
			bool? flag = builder.EvaluateCondition(doWhileStatement.Condition);
			if (flag != false)
			{
				Connect(controlFlowNode2, controlFlowNode3, ControlFlowEdgeType.ConditionTrue);
			}
		...
		public override ControlFlowNode VisitForStatement(ForStatement forStatement, ControlFlowNode data)
		{
			...
			ControlFlowNode controlFlowNode6 = forStatement.EmbeddedStatement.AcceptVisitor(this, controlFlowNode5);
			Connect(controlFlowNode6, controlFlowNode4);
			breakTargets.Pop();
			continueTargets.Pop();
			bool? flag = (forStatement.Condition.IsNull ? new bool?(true) : builder.EvaluateCondition(forStatement.Condition));
			if (flag != false)
			{
		...
		public override ControlFlowNode VisitForeachStatement(ForeachStatement foreachStatement, ControlFlowNode data)
		{
			...
			breakTargets.Push(controlFlowNode);
			continueTargets.Push(controlFlowNode2);
			ControlFlowNode controlFlowNode3 = HandleEmbeddedStatement(foreachStatement.EmbeddedStatement, controlFlowNode2);
			Connect(controlFlowNode3, controlFlowNode2);
			breakTargets.Pop();
			continueTargets.Pop();
		...
		{
			ControlFlowNode controlFlowNode = HandleEmbeddedStatement(checkedStatement.Body, data);
			return CreateConnectedEndNode(checkedStatement, controlFlowNode);
		}

		public override ControlFlowNode VisitUncheckedStatement(UncheckedStatement uncheckedStatement, ControlFlowNode data)
		{
			ControlFlowNode controlFlowNode = HandleEmbeddedStatement(uncheckedStatement.Body, data);
			return CreateConnectedEndNode(uncheckedStatement, controlFlowNode);
		}

		public override ControlFlowNode VisitLockStatement(LockStatement lockStatement, ControlFlowNode data)
		{
			ControlFlowNode controlFlowNode = HandleEmbeddedStatement(lockStatement.EmbeddedStatement, data);
			return CreateConnectedEndNode(lockStatement, controlFlowNode);
		}
		...
		public override ControlFlowNode VisitUsingStatement(UsingStatement usingStatement, ControlFlowNode data)
		{
			...
			ControlFlowNode controlFlowNode = HandleEmbeddedStatement(usingStatement.EmbeddedStatement, data);
			return CreateConnectedEndNode(usingStatement, controlFlowNode);
		}
		...
		{
			ControlFlowNode controlFlowNode = HandleEmbeddedStatement(unsafeStatement.Body, data);
			return CreateConnectedEndNode(unsafeStatement, controlFlowNode);
		}

		public override ControlFlowNode VisitFixedStatement(FixedStatement fixedStatement, ControlFlowNode data)
		{
			ControlFlowNode controlFlowNode = HandleEmbeddedStatement(fixedStatement.EmbeddedStatement, data);
			return CreateConnectedEndNode(fixedStatement, controlFlowNode);
		}
	}
	...
	private void AnnotateLeaveEdgesWithTryFinallyBlocks()
	{
		foreach (ControlFlowEdge item in nodes.SelectMany((ControlFlowNode n) => n.Outgoing))
		{
			...
			{
				if (parent is TryCatchStatement tryCatchStatement)
				{
	...
	private bool AreEqualConstants(ResolveResult c1, ResolveResult c2)
	{
		...
		CSharpResolver cSharpResolver = new CSharpResolver(typeResolveContext);
		ResolveResult resolveResult = cSharpResolver.ResolveBinaryOperator(BinaryOperatorType.Equality, c1, c2);
	...
	public static GraphVizGraph ExportGraph(IList<ControlFlowNode> nodes)
	{
		...
		for (int i = 0; i < array.Length; i++)
		{
			...
			case ControlFlowNodeType.BetweenStatements:
				text += nodes[i].NextStatement.DebugToString();
				break;
			case ControlFlowNodeType.EndNode:
				text = text + "End of " + nodes[i].PreviousStatement.DebugToString();
				break;
			case ControlFlowNodeType.LoopCondition:
				text = text + "Condition in " + nodes[i].NextStatement.DebugToString();
				break;
			default:
				text += "?";
				break;
