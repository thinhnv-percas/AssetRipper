...
public sealed class ReachabilityAnalysis
{
	...
	{
		ControlFlowGraphBuilder controlFlowGraphBuilder = new ControlFlowGraphBuilder();
		IList<ControlFlowNode> controlFlowGraph = controlFlowGraphBuilder.BuildControlFlowGraph(statement, resolver, cancellationToken);
		return Create(controlFlowGraph, recursiveDetectorVisitor, cancellationToken);
	}
	...
	{
		ControlFlowGraphBuilder controlFlowGraphBuilder = new ControlFlowGraphBuilder();
		IList<ControlFlowNode> controlFlowGraph = controlFlowGraphBuilder.BuildControlFlowGraph(statement, resolver, typeResolveContext, cancellationToken);
		return Create(controlFlowGraph, null, cancellationToken);
	}
