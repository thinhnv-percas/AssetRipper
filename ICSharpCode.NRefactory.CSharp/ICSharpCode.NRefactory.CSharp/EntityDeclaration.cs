...
public abstract class EntityDeclaration : AstNode
{
	...
	internal static void SetModifiers(AstNode node, Modifiers newValue)
	{
		...
		AstNode astNode = node.GetChildrenByRole(AttributeRole).LastOrDefault();
		IEnumerable<Modifiers> enumerable = ((node is TypeDeclaration) ? CSharpModifierToken.TypeModifiers : CSharpModifierToken.AllModifiers);
		foreach (Modifiers m in enumerable)
		{
			if ((m & newValue) != Modifiers.None)
			{
				...
				{
					astNode = node.GetChildrenByRole(ModifierRole).First((CSharpModifierToken t) => t.Modifier == m);
				}
			}
			else if ((m & modifiers) != Modifiers.None)
			{
				node.GetChildrenByRole(ModifierRole).First((CSharpModifierToken t) => t.Modifier == m).Remove();
			}
		}
	}
