...
public class UsingAliasDeclaration : AstNode
{
	...
	public Identifier AliasToken => GetChildByRole(AliasRole);
	...
	public UsingAliasDeclaration(string alias, string nameSpace)
	{
		...
		SimpleType simpleType = new SimpleType(nameSpace);
		simpleType.AddAnnotation(BoxedTextColor.Namespace);
		AddChild(simpleType, ImportRole);
	}
	...
	{
		if (other is UsingAliasDeclaration usingAliasDeclaration && AstNode.MatchString(Alias, usingAliasDeclaration.Alias))
		{
			return Import.DoMatch(usingAliasDeclaration.Import, match);
		}
		return false;
	}
}
