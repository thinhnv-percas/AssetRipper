...
public abstract class AstNode : AbstractAnnotatable, IFreezable, INode, ICloneable
{
	...
	{
		string fileName = (((Ancestors.LastOrDefault() ?? this) is SyntaxTree syntaxTree) ? syntaxTree.FileName : null);
	...
	public IEnumerable<AstNode> GetNodesBetween(TextLocation start, TextLocation end)
	{
		...
		while (astNode != null)
		{
			...
			{
				next = ((!(astNode.EndLocation <= start)) ? astNode.FirstChild : astNode.GetNextNode());
			}
			else
			{
				next = astNode.GetNextNode();
				yield return astNode;
			}
