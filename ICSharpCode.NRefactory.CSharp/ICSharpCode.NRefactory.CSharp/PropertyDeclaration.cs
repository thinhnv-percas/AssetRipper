...
public class PropertyDeclaration : EntityDeclaration
{
	...
	{
		if (other is PropertyDeclaration propertyDeclaration && AstNode.MatchString(Name, propertyDeclaration.Name) && MatchAttributesAndModifiers(propertyDeclaration, match) && ReturnType.DoMatch(propertyDeclaration.ReturnType, match) && PrivateImplementationType.DoMatch(propertyDeclaration.PrivateImplementationType, match) && Getter.DoMatch(propertyDeclaration.Getter, match) && Setter.DoMatch(propertyDeclaration.Setter, match))
		{
			return Variables.DoMatch(propertyDeclaration.Variables, match);
		}
		return false;
	}
}
