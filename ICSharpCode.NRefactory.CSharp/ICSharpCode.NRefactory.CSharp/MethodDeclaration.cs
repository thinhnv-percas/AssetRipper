...
public class MethodDeclaration : EntityDeclaration
{
	...
	{
		if (other is MethodDeclaration methodDeclaration && AstNode.MatchString(Name, methodDeclaration.Name) && MatchAttributesAndModifiers(methodDeclaration, match) && ReturnType.DoMatch(methodDeclaration.ReturnType, match) && PrivateImplementationType.DoMatch(methodDeclaration.PrivateImplementationType, match) && TypeParameters.DoMatch(methodDeclaration.TypeParameters, match) && Parameters.DoMatch(methodDeclaration.Parameters, match) && Constraints.DoMatch(methodDeclaration.Constraints, match))
		{
