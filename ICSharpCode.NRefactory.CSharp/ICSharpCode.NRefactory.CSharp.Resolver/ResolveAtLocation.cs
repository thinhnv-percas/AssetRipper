...
public static class ResolveAtLocation
{
	...
	public static ResolveResult Resolve(Lazy<ICompilation> compilation, CSharpUnresolvedFile unresolvedFile, SyntaxTree syntaxTree, TextLocation location, out AstNode node, CancellationToken cancellationToken = default(CancellationToken))
	{
		...
		if (node.Parent is UsingAliasDeclaration && node.Role == UsingAliasDeclaration.AliasRole)
		{
			...
			return cSharpAstResolver.Resolve(((UsingAliasDeclaration)node.Parent).Import, cancellationToken);
		}
		...
		CSharpAstResolver cSharpAstResolver2 = new CSharpAstResolver(compilation.Value, syntaxTree, unresolvedFile);
		ResolveResult resolveResult = cSharpAstResolver2.Resolve(node, cancellationToken);
		if (resolveResult is MethodGroupResolveResult methodGroupResolveResult)
		{
			if (invocationExpression != null)
			{
				return cSharpAstResolver2.Resolve(invocationExpression);
			}
			if (node is Expression)
			{
				Conversion conversion = cSharpAstResolver2.GetConversion((Expression)node, cancellationToken);
