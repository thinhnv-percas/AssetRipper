...
public class CSharpResolver : ICodeContext, ITypeResolveContext, ICompilationProvider
{
	...
	private sealed class LiftedUserDefinedOperator : SpecializedMethod, OverloadResolution.ILiftedOperator, IParameterizedMember, IMember, IEntity, ISymbol, ICompilationProvider, INamedElement, IHasAccessibility
	{
			...
			: base((IMethod)nonLiftedMethod.MemberDefinition, nonLiftedMethod.Substitution)
		{
			...
			MakeNullableVisitor visitor = new MakeNullableVisitor(nonLiftedMethod.Compilation, nonLiftedMethod.Substitution);
			base.Parameters = CreateParameters(visitor);
			...
			{
				base.ReturnType = nonLiftedMethod.ReturnType.AcceptVisitor(visitor);
			}
		}

		public override bool Equals(object obj)
		{
			if (obj is LiftedUserDefinedOperator liftedUserDefinedOperator)
			{
	...
	public IEnumerable<IVariable> LocalVariables => localVariableStack.Where((IVariable v) => v != null);
	...
	public ResolveResult ResolveUnaryOperator(UnaryOperatorType op, ResolveResult expression)
	{
		...
		if (overloadableOperatorName == null)
		{
			...
			case UnaryOperatorType.Await:
			{
				...
				IMethod method;
				IType resultType;
				if (memberLookup.Lookup(resolveResult, "GetResult", EmptyList<IType>.Instance, isInvocation: true) is MethodGroupResolveResult methodGroupResolveResult)
				{
				...
				List<IMethod> list = (from x in resolveResult.Type.GetMethods()
					where x.ImplementedInterfaceMembers.Select((IMember y) => y.MemberDefinition).Contains(interfaceUnsafeOnCompleted)
					select x).ToList();
				if (list.Count == 0)
				{
					list = (from x in resolveResult.Type.GetMethods()
						where x.ImplementedInterfaceMembers.Select((IMember y) => y.MemberDefinition).Contains(interfaceOnCompleted)
						select x).ToList();
					if (list.Count == 1)
					{
		...
		case UnaryOperatorType.BitNot:
			if (type2.Kind == TypeKind.Enum)
			{
				...
				{
					IType type3 = compilation.FindType(expression.ConstantValue.GetType());
					ConstantResolveResult expression2 = new ConstantResolveResult(type3, expression.ConstantValue);
	...
	public ResolveResult ResolveBinaryOperator(BinaryOperatorType op, ResolveResult lhs, ResolveResult rhs)
	{
		...
		if (type.Kind != TypeKind.Null || type2.IsReferenceType != false)
		{
			bool? isReferenceType = type.IsReferenceType;
			bool flag = false;
			if (isReferenceType == true != flag || !isReferenceType.HasValue || type2.Kind != TypeKind.Null)
			{
				goto IL_01a2;
			}
		}
		...
		case BinaryOperatorType.LessThanOrEqual:
			...
			if (op == BinaryOperatorType.Equality || op == BinaryOperatorType.InEquality)
			{
				bool? isReferenceType = type.IsReferenceType;
				bool flag = true;
				if (isReferenceType == true == flag && isReferenceType.HasValue && type2.IsReferenceType == true)
				{
	...
	private IList<List<IMethod>> GetAllExtensionMethods(MemberLookup lookup)
	{
		...
		for (ResolvedUsingScope resolvedUsingScope = currentUsingScope; resolvedUsingScope != null; resolvedUsingScope = resolvedUsingScope.Parent)
		{
			...
			if (obj != null)
			{
				list2 = GetExtensionMethods(lookup, obj).ToList();
				if (list2.Count > 0)
				{
					list.Add(list2);
				}
	...
	private List<IParameter> CreateParameters(ResolveResult[] arguments, string[] argumentNames)
	{
		...
		for (int i = 0; i < arguments.Length; i++)
		{
			...
			{
				list.Add(new DefaultParameter(arguments[i].Type, argumentNames[i], null, default(DomRegion), null, byReferenceResolveResult.IsRef, byReferenceResolveResult.IsOut, isParams: false, isOptional: false, null, byReferenceResolveResult.IsIn));
				continue;
			}
	...
	private static string GuessParameterName(ResolveResult rr)
	{
		...
		if (rr is UnknownMemberResolveResult unknownMemberResolveResult)
		{
			return unknownMemberResolveResult.MemberName;
		}
		if (rr is MethodGroupResolveResult methodGroupResolveResult)
		{
	...
	public ResolveResult ResolveIndexer(ResolveResult target, ResolveResult[] arguments, string[] argumentNames = null)
	{
		...
		{
			MemberLookup memberLookup = CreateMemberLookup();
			IList<MethodListWithDeclaringType> list = memberLookup.LookupIndexers(target);
			if (arguments.Any((ResolveResult a) => a.Type.Kind == TypeKind.Dynamic))
			{
				...
				List<IParameterizedMember> list2 = (from m in list.SelectMany((MethodListWithDeclaringType x) => x)
	...
	public ResolveResult ResolveSizeOf(IType type)
	{
		...
		switch (ReflectionHelper.GetTypeCode(type2))
		{
