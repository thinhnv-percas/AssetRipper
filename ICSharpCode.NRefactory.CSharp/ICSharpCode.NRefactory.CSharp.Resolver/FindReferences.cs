...
public sealed class FindReferences
{
	...
	private sealed class FindTypeDefinitionReferencesNavigator : FindReferenceNavigator
	{
		...
		internal override bool CanMatch(AstNode node)
		{
			...
			if (node is SimpleType simpleType)
			{
				if (searchTerm != null)
				{
			...
			if (node is MemberType memberType)
			{
				if (searchTerm != null)
				{
	...
	private class FindMemberReferencesNavigator : FindReferenceNavigator
	{
		...
		internal override bool CanMatch(AstNode node)
		{
			...
			if (node is PointerReferenceExpression pointerReferenceExpression)
			{
	...
	private sealed class FindEnumeratorCurrentReferencesNavigator : FindReferenceNavigator
	{
		...
		internal override bool IsMatch(ResolveResult rr)
		{
			if (rr is ForEachResolveResult { CurrentProperty: not null } forEachResolveResult)
			{
	...
	private sealed class FindAwaiterIsCompletedReferencesNavigator : FindReferenceNavigator
	{
		...
		internal override bool IsMatch(ResolveResult rr)
		{
			if (rr is AwaitResolveResult { IsCompletedProperty: not null } awaitResolveResult)
			{
	...
	private sealed class FindMethodReferences : FindReferenceNavigator
	{
		...
		internal override bool CanMatch(AstNode node)
		{
			...
			if (node is InvocationExpression invocationExpression)
			{
				...
				if (expression2 is PointerReferenceExpression pointerReferenceExpression)
				{
		...
		internal override bool IsMatch(ResolveResult rr)
		{
			...
			{
				if (rr is ForEachResolveResult forEachResolveResult)
				{
				...
				if (rr is AwaitResolveResult awaitResolveResult)
				{
	...
	private sealed class FindExplicitOperatorNavigator : FindReferenceNavigator
	{
		...
		internal override bool IsMatch(ResolveResult rr)
		{
			if (rr is ConversionResolveResult conversionResolveResult && conversionResolveResult.Conversion.IsUserDefined)
			{
	...
	private class FindLocalReferencesNavigator : FindReferenceNavigator
	{
		...
		internal override bool IsMatch(ResolveResult rr)
		{
			if (rr is LocalResolveResult localResolveResult && localResolveResult.Variable.Name == variable.Name)
			{
	...
	private class FindTypeParameterReferencesNavigator : FindReferenceNavigator
	{
		...
		internal override bool CanMatch(AstNode node)
		{
			...
			if (node is TypeParameterDeclaration typeParameterDeclaration)
			{
		...
		internal override bool IsMatch(ResolveResult rr)
		{
			if (rr is TypeResolveResult typeResolveResult && typeResolveResult.Type.Kind == TypeKind.TypeParameter)
			{
	...
	private sealed class FindNamespaceNavigator : FindReferenceNavigator
	{
		...
		internal override bool CanMatch(AstNode node)
		{
			if (node is NamespaceDeclaration namespaceDeclaration && namespaceDeclaration.FullName.StartsWith(ns.FullName, StringComparison.Ordinal))
			{
				return true;
			}
			if (node is UsingDeclaration usingDeclaration && usingDeclaration.Namespace == ns.FullName)
			{
				return true;
			}
			if (node is SimpleType simpleType && simpleType.Identifier == ns.Name)
			{
			...
			if (node is MemberType memberType && memberType.MemberName == ns.Name)
			{
			...
			if (node is IdentifierExpression identifierExpression && identifierExpression.Identifier == ns.Name)
			{
				return true;
			}
			if (node is MemberReferenceExpression memberReferenceExpression && memberReferenceExpression.MemberName == ns.Name)
			{
				return true;
			}
			return false;
		...
		internal override bool IsMatch(ResolveResult rr)
		{
			if (rr is NamespaceResolveResult namespaceResolveResult)
			{
				return namespaceResolveResult.NamespaceName.StartsWith(ns.FullName, StringComparison.Ordinal);
			}
			return false;
		}
	}
	...
	private class FindParameterReferencesNavigator : FindReferenceNavigator
	{
		...
		internal override bool CanMatch(AstNode node)
		{
			...
			if (node is NamedArgumentExpression namedArgumentExpression)
			{
		...
		internal override bool IsMatch(ResolveResult rr)
		{
			...
			if (rr is NamedArgumentResolveResult namedArgumentResolveResult && namedArgumentResolveResult.Parameter.Name == parameter.Name)
			{
	...
	public IList<IFindReferenceSearchScope> GetSearchScopes(ISymbol symbol)
	{
		...
		case SymbolKind.Namespace:
			return new SearchScope[1] { GetSearchScopeForNamespace((INamespace)symbol) };
		case SymbolKind.TypeParameter:
			return new SearchScope[1] { GetSearchScopeForTypeParameter((ITypeParameter)symbol) };
		default:
		{
			...
			if (additionalScope != null)
			{
				...
				return new SearchScope[2] { searchScope, additionalScope };
			}
			return new SearchScope[1] { searchScope };
		}
		}
	}
	...
	public IEnumerable<CSharpUnresolvedFile> GetInterestingFiles(IFindReferenceSearchScope searchScope, ICompilation compilation)
	{
		...
		if (!(compilation.MainAssembly.UnresolvedAssembly is IProjectContent projectContent))
		{
		...
		if (searchScope.TopLevelTypeDefinition != null)
		{
			...
			case Accessibility.Private:
				...
				{
					return typeDefinition.Parts.Select((IUnresolvedTypeDefinition p) => p.UnresolvedFile).OfType<CSharpUnresolvedFile>().Distinct();
				}
				return EmptyList<CSharpUnresolvedFile>.Instance;
	...
	public void FindReferencesInFile(IFindReferenceSearchScope searchScope, CSharpUnresolvedFile unresolvedFile, SyntaxTree syntaxTree, ICompilation compilation, FoundReferenceCallback callback, CancellationToken cancellationToken)
	{
		...
		FindReferencesInFile(new IFindReferenceSearchScope[1] { searchScope }, unresolvedFile, syntaxTree, compilation, callback, cancellationToken);
	}
	...
	public void FindReferencesInFile(IList<IFindReferenceSearchScope> searchScopes, CSharpUnresolvedFile unresolvedFile, SyntaxTree syntaxTree, ICompilation compilation, FoundReferenceCallback callback, CancellationToken cancellationToken)
	{
		...
		foreach (IResolveVisitorNavigator resolveVisitorNavigator in array2)
		{
			if (resolveVisitorNavigator is FindReferenceNavigator findReferenceNavigator)
			{
				findReferenceNavigator.NavigatorDone(cSharpAstResolver, cancellationToken);
			}
		}
	}
	...
	private SearchScope FindMemberReferences(IEntity member, Func<IMember, FindMemberReferencesNavigator> factory)
	{
		...
		return new SearchScope(name, delegate(ICompilation compilation)
		{
	...
	{
		SearchScope searchScope = new SearchScope((ICompilation _003Cp0_003E) => new FindLocalReferencesNavigator(variable));
		searchScope.fileName = variable.Region.FileName;
		return searchScope;
	}
	...
	{
		SearchScope searchScope = new SearchScope((ICompilation _003Cp0_003E) => new FindParameterReferencesNavigator(parameter));
