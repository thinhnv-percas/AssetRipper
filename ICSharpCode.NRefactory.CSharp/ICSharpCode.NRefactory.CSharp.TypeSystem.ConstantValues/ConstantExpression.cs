...
public abstract class ConstantExpression : IConstantValue
{
	...
	public ResolveResult Resolve(ITypeResolveContext context)
	{
		...
		if (context.CurrentAssembly != context.Compilation.MainAssembly && context.CurrentAssembly is IProjectContent project)
		{
			ICompilation compilation = context.Compilation.SolutionSnapshot.GetCompilation(project);
			if (compilation != null)
			{
				CSharpTypeResolveContext context3 = MapToNestedCompilation(context2, compilation);
				ResolveResult rr = Resolve(new CSharpResolver(context3));
