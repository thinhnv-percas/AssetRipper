	...
	private sealed class CSharpResolvedAttribute : IAttribute
	{
		...
		IList<ResolveResult> IAttribute.PositionalArguments
		{
			get
			{
				...
				InvocationResolveResult invocationResolveResult = GetCtorInvocation();
				return LazyInit.GetOrSet(newValue: (invocationResolveResult == null) ? EmptyList<ResolveResult>.Instance : invocationResolveResult.GetArgumentsForCall(), target: ref positionalArguments);
			}
		}
