...
public class TypeSystemConvertVisitor : DepthFirstAstVisitor<IUnresolvedEntity>
{
	private sealed class ConstantValueBuilder : DepthFirstAstVisitor<ConstantExpression>
	{
		...
		public override ConstantExpression VisitUnaryOperatorExpression(UnaryOperatorExpression unaryOperatorExpression)
		{
			...
			if ((uint)(unaryOperatorType - 1) <= 3u)
			{
		...
		public override ConstantExpression VisitObjectCreateExpression(ObjectCreateExpression objectCreateExpression)
		{
			if (!objectCreateExpression.Arguments.Any())
			{
				...
				case "System.Decimal":
				case "decimal":
					return new PrimitiveConstantExpression(KnownTypeReference.Decimal, 0m);
				}
			}
			return null;
		}
	...
	{
		if (ConvertTypeReference(usingDeclaration.Import, NameLookupMode.TypeInUsingDeclaration) is TypeOrNamespaceReference item)
		{
	...
	{
		if (ConvertTypeReference(usingDeclaration.Import, NameLookupMode.TypeInUsingDeclaration) is TypeOrNamespaceReference value)
		{
			usingScope.UsingAliases.Add(new KeyValuePair<string, TypeOrNamespaceReference>(usingDeclaration.Alias, value));
		}
		return null;
	}
	...
	public override IUnresolvedEntity VisitFieldDeclaration(FieldDeclaration fieldDeclaration)
	{
		...
		foreach (VariableInitializer variable in fieldDeclaration.Variables)
		{
			...
			defaultUnresolvedField.IsVolatile = (modifiers & Modifiers.Volatile) != 0;
			defaultUnresolvedField.IsReadOnly = (modifiers & Modifiers.Readonly) != 0;
			defaultUnresolvedField.ReturnType = ConvertTypeReference(fieldDeclaration.ReturnType);
			if ((modifiers & Modifiers.Const) != Modifiers.None)
			{
	...
	public override IUnresolvedEntity VisitEnumMemberDeclaration(EnumMemberDeclaration enumMemberDeclaration)
	{
		...
		DomRegion region = (defaultUnresolvedField.BodyRegion = MakeRegion(enumMemberDeclaration));
		defaultUnresolvedField.Region = region;
		...
		else if (!(currentTypeDefinition.Members.LastOrDefault() is DefaultUnresolvedField { ConstantValue: not null } defaultUnresolvedField2))
		{
	...
	public override IUnresolvedEntity VisitMethodDeclaration(MethodDeclaration methodDeclaration)
	{
		...
		ConvertAttributes(defaultUnresolvedMethod.Attributes, methodDeclaration.Attributes.Where((AttributeSection s) => s.AttributeTarget != "return"));
		ConvertAttributes(defaultUnresolvedMethod.ReturnTypeAttributes, methodDeclaration.Attributes.Where((AttributeSection s) => s.AttributeTarget == "return"));
	...
	private void ConvertTypeParameters(IList<IUnresolvedTypeParameter> output, AstNodeCollection<TypeParameterDeclaration> typeParameters, AstNodeCollection<Constraint> constraints, SymbolKind ownerType)
	{
		...
		{
			DefaultUnresolvedTypeParameter defaultUnresolvedTypeParameter = new DefaultUnresolvedTypeParameter(ownerType, count++, typeParameter.Name);
		...
		foreach (Constraint constraint in constraints)
		{
			foreach (DefaultUnresolvedTypeParameter item in list)
			{
				...
				{
					if (baseType is PrimitiveType primitiveType)
					{
	...
	public override IUnresolvedEntity VisitOperatorDeclaration(OperatorDeclaration operatorDeclaration)
	{
		...
		ConvertAttributes(defaultUnresolvedMethod.Attributes, operatorDeclaration.Attributes.Where((AttributeSection s) => s.AttributeTarget != "return"));
		ConvertAttributes(defaultUnresolvedMethod.ReturnTypeAttributes, operatorDeclaration.Attributes.Where((AttributeSection s) => s.AttributeTarget == "return"));
	...
	public override IUnresolvedEntity VisitConstructorDeclaration(ConstructorDeclaration constructorDeclaration)
	{
		...
		bool flag = (modifiers & Modifiers.Static) != 0;
	...
	{
		DefaultUnresolvedMethod defaultUnresolvedMethod = new DefaultUnresolvedMethod(currentTypeDefinition, name);
		defaultUnresolvedMethod.SymbolKind = SymbolKind.Accessor;
		defaultUnresolvedMethod.AccessorOwner = ev;
		defaultUnresolvedMethod.Region = ev.BodyRegion;
		defaultUnresolvedMethod.BodyRegion = DomRegion.Empty;
		defaultUnresolvedMethod.Accessibility = ev.Accessibility;
		defaultUnresolvedMethod.IsAbstract = ev.IsAbstract;
		defaultUnresolvedMethod.IsOverride = ev.IsOverride;
		defaultUnresolvedMethod.IsSealed = ev.IsSealed;
		defaultUnresolvedMethod.IsStatic = ev.IsStatic;
		defaultUnresolvedMethod.IsSynthetic = ev.IsSynthetic;
		defaultUnresolvedMethod.IsVirtual = ev.IsVirtual;
		defaultUnresolvedMethod.HasBody = true;
		defaultUnresolvedMethod.ReturnType = KnownTypeReference.Void;
		defaultUnresolvedMethod.Parameters.Add(valueParameter);
		return defaultUnresolvedMethod;
	}
	...
	{
		return (modifiers & Modifiers.VisibilityMask) switch
		{
			Modifiers.Private => Accessibility.Private, 
			Modifiers.Internal => Accessibility.Internal, 
			Modifiers.Internal | Modifiers.Protected => Accessibility.ProtectedOrInternal, 
	...
	private CSharpAttribute ConvertAttribute(Attribute attr)
	{
		...
		{
			if (argument is NamedArgumentExpression namedArgumentExpression)
			{
			...
			else if (argument is NamedExpression namedExpression)
			{
	...
	internal static IConstantValue ConvertConstantValue(ITypeReference targetType, AstNode expression, IUnresolvedTypeDefinition parentTypeDefinition, IUnresolvedMethod parentMethodDefinition, UsingScope parentUsingScope, InterningProvider interningProvider)
	{
		...
		if (constantExpression is PrimitiveConstantExpression primitiveConstantExpression && primitiveConstantExpression.Type == targetType)
		{
	...
	private void ConvertParameters(IList<IUnresolvedParameter> outputList, IEnumerable<ParameterDeclaration> parameters)
	{
		foreach (ParameterDeclaration parameter in parameters)
		{
			...
			case ParameterModifier.In:
				defaultUnresolvedParameter.IsIn = true;
	...
	internal static IList<ITypeReference> GetParameterTypes(IEnumerable<ParameterDeclaration> parameters, InterningProvider interningProvider)
	{
		...
		foreach (ParameterDeclaration parameter in parameters)
		{
			...
			if (parameter.ParameterModifier == ParameterModifier.In || parameter.ParameterModifier == ParameterModifier.Ref || parameter.ParameterModifier == ParameterModifier.Out)
			{
	...
	private void AddXmlDocumentation(IUnresolvedEntity entity, AstNode entityDeclaration)
	{
		...
		{
			if (astNode is Comment { IsDocumentation: not false } comment)
			{
