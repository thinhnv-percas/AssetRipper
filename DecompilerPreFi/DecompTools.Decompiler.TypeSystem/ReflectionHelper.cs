...
public static class ReflectionHelper
{
	...
	public static ITypeReference ToTypeReference(this Type type)
	{
		...
		if (type.IsGenericType && !type.IsGenericTypeDefinition)
		{
			...
			bool flag = true;
			for (int i = 0; i < genericArguments.Length; i = checked(i + 1))
			{
		...
		IModuleReference module = new DefaultAssemblyReference(type.Assembly.FullName);
		string name2 = SplitTypeParameterCountFromReflectionName(type.Name, out var typeParameterCount2);
		return new GetClassTypeReference(module, type.Namespace, name2, typeParameterCount2);
	}
	...
	public static string SplitTypeParameterCountFromReflectionName(string reflectionName, out int typeParameterCount)
	{
		...
		string s = reflectionName.Substring(checked(num + 1));
	...
	private static ITypeReference ParseReflectionName(string reflectionTypeName, ref int pos, bool local = false)
	{
		if (pos == reflectionTypeName.Length)
		{
			...
			while (pos < reflectionTypeName.Length)
			{
				...
				case '[':
					...
					if (reflectionTypeName[pos] != ']' && reflectionTypeName[pos] != ',')
					{
						...
						bool flag = true;
						while (flag || (pos < reflectionTypeName.Length && reflectionTypeName[pos] == ','))
						{
							if (flag)
							{
								flag = false;
							...
							{
								list.Add(ParseReflectionName(reflectionTypeName, ref pos, local: true));
							}
						}
						if (pos >= reflectionTypeName.Length || reflectionTypeName[pos] != ']')
						{
	...
	{
		IModuleReference module = ((assemblyName == null) ? null : new DefaultAssemblyReference(assemblyName));
		int num = typeName.LastIndexOf('.');
		if (num < 0)
		{
			return new GetClassTypeReference(module, string.Empty, typeName, tpc);
		}
		return new GetClassTypeReference(module, typeName.Substring(0, num), typeName.Substring(checked(num + 1)), tpc);
	}
