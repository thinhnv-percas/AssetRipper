...
public class CSharpOutputVisitor : IAstVisitor
{
	...
	private static readonly HashSet<string> unconditionalKeywords;
	...
	public CSharpOutputVisitor(TextWriter textWriter, CSharpFormattingOptions formattingPolicy)
	{
		...
		writer = TokenWriter.Create(textWriter, formattingPolicy.IndentationString);
	...
	{
		LPar();
		if (Enumerable.Any<AstNode>(list))
		{
			Space(spaceWithin);
	...
	protected virtual void WriteCommaSeparatedListInBrackets(IEnumerable<ParameterDeclaration> list, bool spaceWithin)
	{
		...
		if (Enumerable.Any<ParameterDeclaration>(list))
		{
			Space(spaceWithin);
			WriteCommaSeparatedList(list);
			Space(spaceWithin);
	...
	{
		WriteToken(Roles.LBracket);
		if (Enumerable.Any<Expression>(list))
		{
	...
	{
		int num = 0;
		MemberReferenceExpression memberReferenceExpression = expr;
		while (memberReferenceExpression.Target is InvocationExpression { Target: MemberReferenceExpression target } && num < 4)
		{
	...
	{
		int callChainLengthLimited = GetCallChainLengthLimited(expr);
	...
	public static bool IsKeyword(string identifier, AstNode context)
	{
		...
		{
			return Enumerable.Any<AstNode>(context.Ancestors, (Func<AstNode, bool>)((AstNode ancestor) => ancestor is QueryExpression));
		}
		if (identifier == "await")
		{
	...
	{
		if (Enumerable.Any<AstType>(typeArguments))
		{
	...
	{
		if (Enumerable.Any<TypeParameterDeclaration>(typeParameters))
		{
			WriteToken(Roles.LChevron);
	...
	protected virtual void WriteEmbeddedStatement(Statement embeddedStatement, NewLinePlacement nlp = NewLinePlacement.NewLine)
	{
		...
		else if (embeddedStatement is BlockStatement blockStatement)
		{
	...
	public virtual void VisitArrayInitializerExpression(ArrayInitializerExpression arrayInitializerExpression)
	{
		...
		if (arrayInitializerExpression.Elements.Count == 1 && IsObjectOrCollectionInitializer(arrayInitializerExpression.Parent) && !CanBeConfusedWithObjectInitializer(Enumerable.Single<Expression>((IEnumerable<Expression>)arrayInitializerExpression.Elements)) && arrayInitializerExpression.LBraceToken.IsNull)
		{
			Enumerable.Single<Expression>((IEnumerable<Expression>)arrayInitializerExpression.Elements).AcceptVisitor(this);
		}
		else
		{
	...
	{
		return expr is AssignmentExpression assignmentExpression && assignmentExpression.Operator == AssignmentOperatorType.Assign;
	}
	...
	public virtual void VisitInvocationExpression(InvocationExpression invocationExpression)
	{
		...
		if (!(invocationExpression.Parent is MemberReferenceExpression) && invocationExpression.Target is MemberReferenceExpression expr && GetCallChainLengthLimited(expr) >= 3)
		{
			writer.Unindent();
		}
	...
	public virtual void VisitLambdaExpression(LambdaExpression lambdaExpression)
	{
		...
		{
			Enumerable.Single<ParameterDeclaration>((IEnumerable<ParameterDeclaration>)lambdaExpression.Parameters).AcceptVisitor(this);
		}
		Space();
	...
	protected bool LambdaNeedsParenthesis(LambdaExpression lambdaExpression)
	{
		...
		return !parameterDeclaration.Type.IsNull || parameterDeclaration.ParameterModifier != ParameterModifier.None;
	}
	...
	public virtual void VisitInterpolatedStringExpression(InterpolatedStringExpression interpolatedStringExpression)
	{
		...
		foreach (InterpolatedStringContent item in interpolatedStringExpression.Content)
		{
			item.AcceptVisitor(this);
		}
	...
	public virtual void VisitStackAllocExpression(StackAllocExpression stackAllocExpression)
	{
		...
		WriteCommaSeparatedListInBrackets(new Expression[1] { stackAllocExpression.CountExpression });
	...
	public virtual void VisitUnaryOperatorExpression(UnaryOperatorExpression unaryOperatorExpression)
	{
		...
		if (unaryOperatorType == UnaryOperatorType.Await)
		{
			WriteKeyword(operatorRole);
		}
		else if (!IsPostfixOperator(unaryOperatorType) && operatorRole != null)
		{
			WriteToken(operatorRole);
		}
		unaryOperatorExpression.Expression.AcceptVisitor(this);
		if (IsPostfixOperator(unaryOperatorType))
		{
			WriteToken(operatorRole);
		}
	...
	{
		return op == UnaryOperatorType.PostIncrement || op == UnaryOperatorType.PostDecrement || op == UnaryOperatorType.NullConditional || op == UnaryOperatorType.SuppressNullableWarning;
	}
	...
	public virtual void VisitAttribute(DecompTools.Decompiler.CSharp.Syntax.Attribute attribute)
	{
		StartNode(attribute);
	...
	public virtual void VisitTypeDeclaration(TypeDeclaration typeDeclaration)
	{
		...
		else
		{
			...
			foreach (EntityDeclaration member2 in typeDeclaration.Members)
			{
				if (!flag2)
				{
					for (int i = 0; i < policy.MinimumBlankLinesBetweenMembers; i = checked(i + 1))
					{
						NewLine();
					}
				}
	...
	public virtual void VisitSwitchSection(SwitchSection switchSection)
	{
		...
		bool flag2 = switchSection.Statements.Count == 1 && Enumerable.Single<Statement>((IEnumerable<Statement>)switchSection.Statements) is BlockStatement;
	...
	public virtual void VisitConstructorDeclaration(ConstructorDeclaration constructorDeclaration)
	{
		...
		if (constructorDeclaration.Parent is TypeDeclaration typeDeclaration && typeDeclaration.Name != constructorDeclaration.Name)
		{
	...
	public virtual void VisitDestructorDeclaration(DestructorDeclaration destructorDeclaration)
	{
		...
		if (Enumerable.Any<CSharpModifierToken>(destructorDeclaration.ModifierTokens))
		{
			Space();
		}
		WriteToken(DestructorDeclaration.TildeRole);
		if (destructorDeclaration.Parent is TypeDeclaration typeDeclaration && typeDeclaration.Name != destructorDeclaration.Name)
		{
			WriteIdentifier((Identifier)typeDeclaration.NameToken.Clone());
		}
		else
	...
	private void MaybeNewLinesAfterUsings(AstNode node)
	{
		...
		{
			for (int i = 0; i < policy.MinimumBlankLinesAfterUsings; i = checked(i + 1))
			{
				NewLine();
			}
		}
	...
	public virtual void VisitComposedType(ComposedType composedType)
	{
		...
		for (int i = 0; i < composedType.PointerRank; i = checked(i + 1))
		{
	...
	private void VisitChoice(Choice choice)
	{
		...
		foreach (INode item in (IEnumerable<INode>)choice)
		{
			...
			if (item != Enumerable.Last<INode>((IEnumerable<INode>)choice))
			{
	...
	static CSharpOutputVisitor()
	{
		...
		obj2.Add("by");
		queryKeywords = obj2;
		maxKeywordLength = Enumerable.Max<string>(Enumerable.Concat<string>((IEnumerable<string>)unconditionalKeywords, (IEnumerable<string>)queryKeywords), (Func<string, int>)((string s) => s.Length));
	}
}
