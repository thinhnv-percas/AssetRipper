...
public class DelegateDeclaration : EntityDeclaration
{
	...
	{
		return other is DelegateDeclaration delegateDeclaration && AstNode.MatchString(Name, delegateDeclaration.Name) && MatchAttributesAndModifiers(delegateDeclaration, match) && ReturnType.DoMatch(delegateDeclaration.ReturnType, match) && TypeParameters.DoMatch(delegateDeclaration.TypeParameters, match) && Parameters.DoMatch(delegateDeclaration.Parameters, match) && Constraints.DoMatch(delegateDeclaration.Constraints, match);
	}
}
