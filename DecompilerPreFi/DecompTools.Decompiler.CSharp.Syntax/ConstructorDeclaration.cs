...
public class ConstructorDeclaration : EntityDeclaration
{
	...
	{
		return other is ConstructorDeclaration constructorDeclaration && MatchAttributesAndModifiers(constructorDeclaration, match) && Parameters.DoMatch(constructorDeclaration.Parameters, match) && Initializer.DoMatch(constructorDeclaration.Initializer, match) && Body.DoMatch(constructorDeclaration.Body, match);
	}
}
