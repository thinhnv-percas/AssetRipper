...
public class CSharpInvocationResolveResult : InvocationResolveResult
{
	...
	private readonly IReadOnlyList<int> argumentToParameterMap;
	...
	public override IList<ResolveResult> GetArgumentsForCall()
	{
		...
		checked
		{
			...
			if (IsExpandedForm)
			{
				IType arrayType = Enumerable.Last<IParameter>((IEnumerable<IParameter>)base.Member.Parameters).Type;
			...
			for (int j = 0; j < array.Length; j++)
			{
				if (array[j] == null)
				{
					...
					{
						array[j] = new ConstantResolveResult(base.Member.Parameters[j].Type, base.Member.Parameters[j].GetConstantValue());
					}
					else
					{
