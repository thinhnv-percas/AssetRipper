...
public sealed class DynamicSetIndexInstruction : DynamicInstruction
{
	...
	{
		DynamicSetIndexInstruction dynamicSetIndexInstruction = (DynamicSetIndexInstruction)ShallowClone();
		dynamicSetIndexInstruction.Arguments = new InstructionCollection<ILInstruction>(dynamicSetIndexInstruction, 0);
		dynamicSetIndexInstruction.Arguments.AddRange(Enumerable.Select<ILInstruction, ILInstruction>((IEnumerable<ILInstruction>)Arguments, (Func<ILInstruction, ILInstruction>)((ILInstruction arg) => arg.Clone())));
	...
	{
		return other is DynamicSetIndexInstruction dynamicSetIndexInstruction && ListMatch.DoMatch(Arguments, dynamicSetIndexInstruction.Arguments, ref match);
	}
}
