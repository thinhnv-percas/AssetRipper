...
public sealed class DynamicInvokeInstruction : DynamicInstruction
{
	...
	{
		DynamicInvokeInstruction dynamicInvokeInstruction = (DynamicInvokeInstruction)ShallowClone();
		dynamicInvokeInstruction.Arguments = new InstructionCollection<ILInstruction>(dynamicInvokeInstruction, 0);
		dynamicInvokeInstruction.Arguments.AddRange(Enumerable.Select<ILInstruction, ILInstruction>((IEnumerable<ILInstruction>)Arguments, (Func<ILInstruction, ILInstruction>)((ILInstruction arg) => arg.Clone())));
	...
	{
		return other is DynamicInvokeInstruction dynamicInvokeInstruction && ListMatch.DoMatch(Arguments, dynamicInvokeInstruction.Arguments, ref match);
	}
}
