...
public static class IdStringProvider
{
	public static string GetIdString(this IEntity entity)
	{
		...
		if (member.IsExplicitInterfaceImplementation && member.Name.IndexOf('.') < 0 && Enumerable.Count<IMember>(member.ExplicitlyImplementedInterfaceMembers) == 1)
		{
			AppendTypeName(stringBuilder, Enumerable.First<IMember>(member.ExplicitlyImplementedInterfaceMembers).DeclaringType, explicitInterfaceImpl: true);
			stringBuilder.Append('#');
		}
		stringBuilder.Append(member.Name.Replace('.', '#').Replace('<', '{').Replace('>', '}'));
	...
	private static void AppendTypeName(StringBuilder b, IType type, bool explicitInterfaceImpl)
	{
		...
		case TypeKind.Array:
		{
			...
			{
				for (int i = 0; i < arrayType.Dimensions; i = checked(i + 1))
				{
					if (i > 0)
					{
	...
	private static void AppendTypeParameters(StringBuilder b, IType type, int outerTypeParameterCount, bool explicitInterfaceImpl)
	{
		checked
		{
			...
			{
				b.Append('{');
				IReadOnlyList<IType> typeArguments = parameterizedType.TypeArguments;
	...
	public static IMemberReference ParseMemberIdString(string memberIdString)
	{
		...
		int num2 = memberIdString.LastIndexOf('.', checked(num - 1));
		if (num2 < 0)
		{
