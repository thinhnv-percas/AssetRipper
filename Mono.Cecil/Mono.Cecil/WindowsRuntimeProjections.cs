...
internal sealed class WindowsRuntimeProjections
{
	...
	{
		TypeReferenceTreatment typeReferenceTreatment = ((!Projections.TryGetValue(type.Name, out var value) || !(value.WinRTNamespace == type.Namespace)) ? GetSpecialTypeReferenceTreatment(type) : TypeReferenceTreatment.UseProjectionInfo);
