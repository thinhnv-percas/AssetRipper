...
namespace EasyMobile.Internal
{
	...
	internal static class PInvokeCallbackUtil
	{
		...
		internal static IntPtr ToIntPtr<T>(Action<T> callback, Func<IntPtr, T> conversionFunction) where T : InteropObject
		{
			...
			Il2CppRuntime.Boundary("MANAGED", "Method not found @BAC1E4 (EasyMobile.Internal.PInvokeCallbackUtil::ToIntPtr, and 1 more at this address)");
		...
		internal static IntPtr ToIntPtr<T, P>(Action<T, P> callback, Func<IntPtr, T> conversionFunctionT, Func<IntPtr, P> conversionFunctionP) where T : InteropObject where P : InteropObject
		{
			...
			Il2CppRuntime.Boundary("MANAGED", "Method not found @BAC114 (EasyMobile.Internal.PInvokeCallbackUtil::ToIntPtr, and 1 more at this address)");
			IntPtr result = default(IntPtr);
			return result;
		}
		...
		private static T IntPtrToCallback<T>(IntPtr handle, bool unpinHandle) where T : class
		{
			...
			{
				GCHandle gCHandle = GCHandle.FromIntPtr(handle);
				Il2CppRuntime.Boundary("UNKNOWN", "Method not found @F74DF0 (inside System.Runtime.InteropServices.GCHandle::GetTargetHandle +0x14)");
				int num = default(int);
				if (num != 0)
				{
					T val = num as T;
