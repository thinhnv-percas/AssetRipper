...
public class PlayMakerAnimatorMove : PlayMakerProxyBase
{
		...
		do
		{
			...
			if (!(items[num] == null))
			{
				...
				if ((IntPtr)0 != (IntPtr)0 && items[num].Active)
				{
					...
					Cpp2ILHelpers.NoteDecompilerIssue("Unmanaged memory load: [v109 @ X20_v8 (UnityEngine.Object)+18]");
					object obj4 = 0;
					Cpp2ILHelpers.NoteDecompilerIssue("Unmanaged memory load: [v106 @ X8_v15+1E4]");
					if ((IntPtr)0 != (IntPtr)0)
					{
						_ = items[num];
						Cpp2ILHelpers.NoteDecompilerIssue("Unmanaged memory load: [v109 @ X20_v8 (UnityEngine.Object)+18]");
						((Fsm)0).OnAnimatorMove();
					}
				}
			}
	...
	[Address(RVA = "0xE54288", Offset = "0xE54288", Length = "0x4")]
	[NativeSource(Body = "// Approximate reconstruction from native code. Reads as C#; does not compile.\n\tPlayMakerProxyBase::.ctor(this);\n\treturn;\n")]
	public PlayMakerAnimatorMove()
	{
	}
}
