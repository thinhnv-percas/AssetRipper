...
public class PlayMakerAnimatorIK : PlayMakerProxyBase
{
		...
		do
		{
			...
			if (!(items[num] == null))
			{
				...
				if ((IntPtr)0 != (IntPtr)0 && items[num].Active)
				{
					...
					Cpp2ILHelpers.NoteDecompilerIssue("Unmanaged memory load: [v113 @ X21_v8 (UnityEngine.Object)+18]");
					object obj4 = 0;
					Cpp2ILHelpers.NoteDecompilerIssue("Unmanaged memory load: [v110 @ X8_v15+1E5]");
					if ((IntPtr)0 != (IntPtr)0)
					{
						_ = items[num];
						Cpp2ILHelpers.NoteDecompilerIssue("Unmanaged memory load: [v113 @ X21_v8 (UnityEngine.Object)+18]");
						((Fsm)0).OnAnimatorIK(layerIndex);
					}
				}
			}
	...
	[Address(RVA = "0xE540FC", Offset = "0xE540FC", Length = "0x4")]
	[NativeSource(Body = "// Approximate reconstruction from native code. Reads as C#; does not compile.\n\tPlayMakerProxyBase::.ctor(this);\n\treturn;\n")]
	public PlayMakerAnimatorIK()
	{
	}
}
