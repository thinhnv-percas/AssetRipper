...
public class PlayMakerCollisionEnter2D : PlayMakerProxyBase
{
	...
	public void OnCollisionEnter2D(Collision2D collisionInfo)
	{
		...
		do
		{
			if (num < targetFSMs.Count)
			{
				...
				if (!(items[num] == null))
				{
					...
					if ((IntPtr)0 != (IntPtr)0 && items[num].Active)
					{
						...
						Cpp2ILHelpers.NoteDecompilerIssue("Unmanaged memory load: [v113 @ X21_v10 (UnityEngine.Object)+18]");
						object obj4 = 0;
						Cpp2ILHelpers.NoteDecompilerIssue("Unmanaged memory load: [v110 @ X8_v15+13E]");
						if ((IntPtr)0 != (IntPtr)0)
						{
							_ = items[num];
							Cpp2ILHelpers.NoteDecompilerIssue("Unmanaged memory load: [v113 @ X21_v10 (UnityEngine.Object)+18]");
							((Fsm)0).OnCollisionEnter2D(collisionInfo);
						}
					}
				}
			...
			if (Collision2DEventCallback != null)
			{
				Collision2DEventCallback(collisionInfo);
			}
			return;
		}
	...
	[Address(RVA = "0xE5484C", Offset = "0xE5484C", Length = "0x4")]
	[NativeSource(Body = "// Approximate reconstruction from native code. Reads as C#; does not compile.\n\tPlayMakerProxyBase::.ctor(this);\n\treturn;\n")]
	public PlayMakerCollisionEnter2D()
	{
	}
}
