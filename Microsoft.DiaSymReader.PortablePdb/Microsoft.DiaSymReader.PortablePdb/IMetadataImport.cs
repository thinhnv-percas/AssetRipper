...
public interface IMetadataImport
{
	...
	void GetTypeDefProps(int typeDefinition, [Out][MarshalAs(UnmanagedType.LPWStr)] StringBuilder qualifiedName, int qualifiedNameBufferLength, out int qualifiedNameLength, [MarshalAs(UnmanagedType.U4)] out TypeAttributes attributes, out int baseType);

	uint GetInterfaceImplProps(uint impl, out uint pointerClass);

	void GetTypeRefProps(int typeReference, out int resolutionScope, [Out][MarshalAs(UnmanagedType.LPWStr)] StringBuilder qualifiedName, int qualifiedNameBufferLength, out int qualifiedNameLength);
	...
	unsafe uint EnumMethods(ref uint handlePointerEnum, uint cl, uint* arrayMethods, uint countMax);
	...
	unsafe uint EnumFields(ref uint handlePointerEnum, uint cl, uint* arrayFields, uint countMax);
	...
	uint GetMethodProps(uint mb, out uint pointerClass, IntPtr stringMethod, uint cchMethod, out uint pchMethod, IntPtr pdwAttr, IntPtr ppvSigBlob, IntPtr pcbSigBlob, IntPtr pulCodeRVA);
	...
	unsafe uint EnumProperties(ref uint handlePointerEnum, uint td, uint* arrayProperties, uint countMax);

	unsafe uint EnumEvents(ref uint handlePointerEnum, uint td, uint* arrayEvents, uint countMax);
	...
	[PreserveSig]
	unsafe int GetSigFromToken(int tkSignature, out byte* ppvSig, out int pcbSig);
	...
	unsafe uint GetMemberProps(uint mb, out uint pointerClass, StringBuilder stringMember, uint cchMember, out uint pchMember, out uint pdwAttr, out byte* ppvSigBlob, out uint pcbSigBlob, out uint pulCodeRVA, out uint pdwImplFlags, out uint pdwCPlusTypeFlag, out void* ppValue);

	unsafe uint GetFieldProps(uint mb, out uint pointerClass, StringBuilder stringField, uint cchField, out uint pchField, out uint pdwAttr, out byte* ppvSigBlob, out uint pcbSigBlob, out uint pdwCPlusTypeFlag, out void* ppValue);

	unsafe uint GetPropertyProps(uint prop, out uint pointerClass, StringBuilder stringProperty, uint cchProperty, out uint pchProperty, out uint pdwPropFlags, out byte* ppvSig, out uint bytePointerSig, out uint pdwCPlusTypeFlag, out void* ppDefaultValue, out uint pcchDefaultValue, out uint pmdSetter, out uint pmdGetter, [MarshalAs(UnmanagedType.LPArray, SizeParamIndex = 14)] uint[] rmdOtherMethod, uint countMax);

	unsafe uint GetParamProps(uint tk, out uint pmd, out uint pulSequence, StringBuilder stringName, uint cchName, out uint pchName, out uint pdwAttr, out uint pdwCPlusTypeFlag, out void* ppValue);

	unsafe uint GetCustomAttributeByName(uint tokenObj, string stringName, out void* ppData);
	...
	unsafe uint GetNativeCallConvFromSig(void* voidPointerSig, uint byteCountSig);
