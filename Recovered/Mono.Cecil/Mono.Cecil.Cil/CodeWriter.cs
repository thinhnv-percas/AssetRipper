...
namespace Mono.Cecil.Cil
{
	internal sealed class CodeWriter : ByteBuffer
	{
		...
		private void WriteUnresolvedMethodBody(MethodDefinition method)
		{
			...
			ByteBuffer buffer = metadata.module.Read(method, (MethodDefinition _, MetadataReader reader) => reader.code).PatchRawMethodBody(method, this, out symbols);
			WriteBytes(buffer);
		...
		{
			if (stack_sizes != null && stack_sizes.TryGetValue(instruction, out int value))
			{
				stack_size = value;
			}
		...
		private static void CopyBranchStackSize(ref Dictionary<Instruction, int> stack_sizes, Instruction target, int stack_size)
		{
			...
			int num = stack_size;
			if (stack_sizes.TryGetValue(target, out int value))
			{
		...
		{
			if (standalone_signatures.TryGetValue(signature, out MetadataToken value))
			{
				return value;
			}
