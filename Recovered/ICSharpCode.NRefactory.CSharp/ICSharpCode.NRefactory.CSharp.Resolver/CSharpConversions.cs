...
namespace ICSharpCode.NRefactory.CSharp.Resolver
{
	public sealed class CSharpConversions
	{
		...
		public Conversion ImplicitConversion(IType fromType, IType toType)
		{
			...
			if (implicitConversionCache.TryGetValue(key, out Conversion value))
			{
				return value;
			}
		...
		private bool ImplicitReferenceConversion(IType fromType, IType toType, int subtypeCheckNestingDepth)
		{
			...
			ArrayType arrayType = fromType as ArrayType;
			if (arrayType != null)
			{
				ArrayType arrayType2 = toType as ArrayType;
				if (arrayType2 != null)
				{
		...
		{
			ParameterizedType parameterizedType = interfaceType as ParameterizedType;
			...
			if (definition != null)
			{
				...
				if (parameterizedType != null && parameterizedType2 != null)
				{
					for (int i = 0; i < definition.TypeParameters.Count; i++)
					{
						...
						switch (definition.TypeParameters[i].Variance)
						{
		...
		{
			List<OperatorInfo> list = (from op in operators
				where op.SourceType.Equals(mostSpecificSource) && op.TargetType.Equals(mostSpecificTarget)
				select op).ToList();
			if (list.Count == 0)
			{
				return Conversion.None;
			...
			if (list.Count != 1)
			{
				if (list.Count((OperatorInfo s) => !s.IsLifted) == 1)
				{
		...
		private Conversion UserDefinedImplicitConversion(ResolveResult fromResult, IType fromType, IType toType)
		{
			...
			{
				IType type = applicableConversionOperators.Any((OperatorInfo op) => op.SourceType.Equals(fromType)) ? fromType : FindMostEncompassedType(from op in applicableConversionOperators
					select op.SourceType);
				if (type == null)
				{
					return Conversion.UserDefinedConversion(applicableConversionOperators[0].Method, isLifted: applicableConversionOperators[0].IsLifted, isImplicit: true, conversionBeforeUserDefinedOperator: Conversion.None, conversionAfterUserDefinedOperator: Conversion.None, isAmbiguous: true);
				}
				IType type2 = applicableConversionOperators.Any((OperatorInfo op) => op.TargetType.Equals(toType)) ? toType : FindMostEncompassingType(from op in applicableConversionOperators
		...
		private Conversion UserDefinedExplicitConversion(ResolveResult fromResult, IType fromType, IType toType)
		{
			...
			if (applicableConversionOperators.Count > 0)
			{
				...
				{
					List<OperatorInfo> source = (from op in applicableConversionOperators
						where (!IsEncompassedBy(fromType, op.SourceType)) ? ImplicitConstantExpressionConversion(fromResult, NullableType.GetUnderlyingType(op.SourceType)) : true
						select op).ToList();
					type = ((!source.Any()) ? FindMostEncompassingType(from op in applicableConversionOperators
						select op.SourceType) : FindMostEncompassedType(from op in source
						select op.SourceType));
				}
				if (type == null)
				{
					return Conversion.UserDefinedConversion(applicableConversionOperators[0].Method, isLifted: applicableConversionOperators[0].IsLifted, isImplicit: false, conversionBeforeUserDefinedOperator: Conversion.None, conversionAfterUserDefinedOperator: Conversion.None, isAmbiguous: true);
				}
				IType type2 = applicableConversionOperators.Any((OperatorInfo op) => op.TargetType.Equals(toType)) ? toType : ((!applicableConversionOperators.Any((OperatorInfo op) => IsEncompassedBy(op.TargetType, toType))) ? FindMostEncompassedType(from op in applicableConversionOperators
					select op.TargetType) : FindMostEncompassingType(from op in applicableConversionOperators
		...
		private static IType UnpackExpressionTreeType(IType type)
		{
			...
			if (parameterizedType != null && parameterizedType.TypeParameterCount == 1 && parameterizedType.Name == "Expression" && parameterizedType.Namespace == "System.Linq.Expressions")
			{
				return parameterizedType.GetTypeArgument(0);
			}
			return type;
		...
		private Conversion MethodGroupConversion(ResolveResult resolveResult, IType toType)
		{
			...
			if (overloadResolution.FoundApplicableCandidate)
			{
				...
				bool num = !overloadResolution.IsAmbiguous && IsDelegateCompatible(method, delegateInvokeMethod, overloadResolution.IsExtensionMethodInvocation);
		...
		{
			ParameterizedType parameterizedType = type as ParameterizedType;
			if (parameterizedType != null && parameterizedType.TypeParameterCount == 1 && parameterizedType.Name == "Task" && parameterizedType.Namespace == "System.Threading.Tasks")
			{
				return parameterizedType.GetTypeArgument(0);
			}
			return null;
