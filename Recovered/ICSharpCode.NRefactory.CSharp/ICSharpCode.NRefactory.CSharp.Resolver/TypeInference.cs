...
namespace ICSharpCode.NRefactory.CSharp.Resolver
{
	public sealed class TypeInference
	{
		...
		{
			return new TypeInference(compilation, conversions)
			{
				algorithm = algorithm,
		...
		public IType[] InferTypeArguments(IList<ITypeParameter> typeParameters, IList<ResolveResult> arguments, IList<IType> parameterTypes, out bool success, IList<IType> classTypeArguments = null)
		{
			...
			try
			{
				...
				PhaseOne();
				success = PhaseTwo();
				return (from tp in this.typeParameters
					select tp.FixedTo ?? SpecialType.UnknownType).ToArray();
			}
			finally
			{
				Reset();
		...
		private bool PhaseTwo()
		{
			...
			{
				ResolveResult resolveResult = arguments[j];
				IType type = parameterTypes[j];
		...
		private IType[] InputTypes(ResolveResult e, IType t)
		{
			...
			if ((lambdaResolveResult != null && lambdaResolveResult.IsImplicitlyTyped) || e is MethodGroupResolveResult)
			{
		...
		private IType[] OutputTypes(ResolveResult e, IType t)
		{
			if (e is LambdaResolveResult || e is MethodGroupResolveResult)
			{
				...
				if (delegateOrExpressionTreeSignature != null)
				{
					return new IType[1]
					{
						delegateOrExpressionTreeSignature.ReturnType
					};
				}
			}
			return emptyTypeArray;
		...
		{
			ParameterizedType parameterizedType = t as ParameterizedType;
		...
		private void CalculateDependencyMatrix()
		{
			...
			for (int i = 0; i < arguments.Length; i++)
			{
				...
				for (int j = 0; j < array.Length; j++)
				{
					array[j].AcceptVisitor(occursInVisitor);
				}
				array = OutputTypes(arguments[i], parameterTypes[i]);
				for (int j = 0; j < array.Length; j++)
				{
					array[j].AcceptVisitor(occursInVisitor2);
				}
				for (int k = 0; k < num; k++)
				{
					for (int l = 0; l < num; l++)
					{
						dependencyMatrix[k, l] |= (occursInVisitor.Occurs[l] && occursInVisitor2.Occurs[k]);
					}
				}
			}
			for (int m = 0; m < num; m++)
		...
		private void MakeOutputTypeInference(ResolveResult e, IType t)
		{
			...
			MethodGroupResolveResult methodGroupResolveResult = e as MethodGroupResolveResult;
		...
		{
			ITypeParameter typeParameter = v as ITypeParameter;
			if (typeParameter != null)
			{
				int index = typeParameter.Index;
				if (index < typeParameters.Length && typeParameters[index].TypeParameter == typeParameter)
				{
		...
		private void MakeLowerBoundInference(IType U, IType V)
		{
			...
			else
			{
				...
				{
					ParameterizedType parameterizedType3 = allBaseType as ParameterizedType;
					if (parameterizedType3 != null && object.Equals(parameterizedType3.GetDefinition(), parameterizedType.GetDefinition()) && parameterizedType3.TypeParameterCount == parameterizedType.TypeParameterCount)
					{
				...
				for (int i = 0; i < parameterizedType2.TypeParameterCount; i++)
				{
					...
					{
						switch (parameterizedType.GetDefinition().TypeParameters[i].Variance)
						{
		...
		private void MakeUpperBoundInference(IType U, IType V)
		{
			...
			else
			{
				...
				{
					ParameterizedType parameterizedType3 = allBaseType as ParameterizedType;
					if (parameterizedType3 != null && object.Equals(parameterizedType.GetDefinition(), parameterizedType3.GetDefinition()) && parameterizedType.TypeParameterCount == parameterizedType3.TypeParameterCount)
					{
						if (parameterizedType2 != null)
						{
							return;
		...
		private IList<IType> FindTypesInBounds(IList<IType> lowerBounds, IList<IType> upperBounds)
		{
			...
			List<IType> candidateTypes = (from c in lowerBounds.Union(upperBounds)
				...
				select c).ToList();
			candidateTypes = (from c in candidateTypes
				where candidateTypes.All((IType o) => conversions.ImplicitConversion(o, c).IsValid)
				select c).ToList();
			...
			foreach (ITypeDefinition candidateDef in list)
			{
				...
				{
					bool success;
					IType[] typeArguments = InferTypeArgumentsFromBounds(candidateDef.TypeParameters, new ParameterizedType(candidateDef, candidateDef.TypeParameters), lowerBounds, upperBounds, out success);
					if (!success)
					{
						continue;
					}
