	...
	public class OverloadResolution
	{
			...
			public int ArgumentsPassedToParamsArray
			{
				get
				{
					...
					if (IsExpandedForm)
					{
						...
						{
							if (argumentToParameterMap[i] == num2)
							{
								num++;
							}
						}
		...
		private sealed class ConstraintValidatingSubstitution : TypeParameterSubstitution
		{
			...
			public override IType VisitParameterizedType(ParameterizedType type)
			{
				...
				if (type2 != type && ConstraintsValid)
				{
					ParameterizedType parameterizedType = type2 as ParameterizedType;
		...
		{
			IList<IParameter> NonLiftedParameters
			{
				get;
			}
		}
		...
		public bool AllowExpandingParams
		{
			get;
			set;
		}

		public bool AllowOptionalParameters
		{
			get;
			set;
		}
		...
		private void RunTypeInference(Candidate candidate)
		{
			...
			IList<IType> classTypeArguments = (candidate.Member.DeclaringType as ParameterizedType)?.TypeArguments;
			if (explicitlyGivenTypeArguments != null)
			{
			...
			else
			{
				...
				candidate.InferredTypes = typeInference.InferTypeArguments(candidate.TypeParameters, arguments, candidate.ParameterTypes, out bool success, classTypeArguments);
				if (!success)
				{
		...
		internal static bool ValidateConstraints(ITypeParameter typeParameter, IType typeArgument, TypeVisitor substitution, CSharpConversions conversions)
		{
			...
			if (typeParameter.HasDefaultConstructorConstraint)
			{
				...
				if (!typeArgument.GetConstructors((IUnresolvedMethod m) => m.Parameters.Count == 0 && m.Accessibility == Accessibility.Public, GetMemberOptions.ReturnMemberDefinitions | GetMemberOptions.IgnoreInheritedMembers).Any())
				{
					return false;
				}
			}
		...
		{
			return (errors & ~(OverloadResolutionErrors.AmbiguousMatch | OverloadResolutionErrors.MethodConstraintsNotSatisfied)) == OverloadResolutionErrors.None;
		}
		...
		private void CheckApplicability(Candidate candidate)
		{
			...
			for (int k = 0; k < arguments.Length; k++)
			{
				...
				ByReferenceResolveResult byReferenceResolveResult = arguments[k] as ByReferenceResolveResult;
		...
		private int MoreSpecificFormalParameters(Candidate c1, Candidate c2)
		{
			...
			return MoreSpecificFormalParameters(from p in c1.Parameters
				select p.Type, from p in c2.Parameters
				select p.Type);
		}
