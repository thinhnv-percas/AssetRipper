	...
	internal sealed class CSharpOperators
	{
		internal class OperatorMethod : IParameterizedMember, IMember, IEntity, ISymbol, ICompilationProvider, INamedElement, IHasAccessibility
		{
			...
			public override string ToString()
			{
				...
				stringBuilder.Append(ReturnType + " operator(");
		...
		private sealed class LambdaBinaryOperatorMethod<T1, T2> : BinaryOperatorMethod
		{
			...
			public override object Invoke(CSharpResolver resolver, object lhs, object rhs)
			{
				...
				return (resolver.CheckForOverflow ? checkedFunc : uncheckedFunc)((T1)resolver.CSharpPrimitiveCast(Type.GetTypeCode(typeof(T1)), lhs), (T2)resolver.CSharpPrimitiveCast(Type.GetTypeCode(typeof(T2)), rhs));
			}
		...
		public OperatorMethod[] CheckedUnaryMinusOperators
		{
			get
			{
				...
				return LazyInit.GetOrSet(ref checkedUnaryMinusOperators, Lift(new LambdaUnaryOperatorMethod<int>(this, (int i) => checked(-i)), new LambdaUnaryOperatorMethod<long>(this, (long i) => checked(-i)), new LambdaUnaryOperatorMethod<float>(this, (float i) => 0f - i), new LambdaUnaryOperatorMethod<double>(this, (double i) => 0.0 - i), new LambdaUnaryOperatorMethod<decimal>(this, (decimal i) => -i)));
			}
		}
		...
		public OperatorMethod[] ValueEqualityOperators
		{
			get
			{
				...
				return LazyInit.GetOrSet(ref valueEqualityOperators, Lift((from c in valueEqualityOperatorsFor
					select new EqualityOperatorMethod(this, c, negate: false)).ToArray()));
			}
		}
		...
		public OperatorMethod[] ValueInequalityOperators
		{
			get
			{
				...
				return LazyInit.GetOrSet(ref valueInequalityOperators, Lift((from c in valueEqualityOperatorsFor
					select new EqualityOperatorMethod(this, c, negate: true)).ToArray()));
			}
		}
		...
		private OperatorMethod[] Lift(params OperatorMethod[] methods)
		{
			...
			{
				OperatorMethod operatorMethod = methods[i].Lift(this);
				if (operatorMethod != null)
				{
					list.Add(operatorMethod);
				}
			}
			return list.ToArray();
		}
