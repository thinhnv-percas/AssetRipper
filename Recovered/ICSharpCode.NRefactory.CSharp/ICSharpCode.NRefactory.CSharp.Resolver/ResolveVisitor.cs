...
namespace ICSharpCode.NRefactory.CSharp.Resolver
{
	internal sealed class ResolveVisitor : IAstVisitor<ResolveResult>
	{
		...
		private sealed class ExplicitlyTypedLambda : LambdaBase
		{
			...
			{
				return "[ExplicitlyTypedLambda " + LambdaExpression + "]";
			}
		...
		private sealed class ImplicitlyTypedLambda : LambdaBase
		{
			...
			public override string ToString()
			{
				return "[ImplicitlyTypedLambda " + LambdaExpression + "]";
			}
		}
		...
		private sealed class LambdaTypeHypothesis : IResolveVisitorNavigator
		{
			...
			internal int CountUnknownParameters()
			{
				...
				for (int i = 0; i < array.Length; i++)
				{
					if (array[i].Kind == TypeKind.Unknown)
					{
						num++;
					}
				}
		...
		{
			internal abstract bool IsUndecided
			{
				get;
			}

			internal abstract AstNode LambdaExpression
			{
				get;
			}

			internal abstract AstNode BodyExpression
			{
				get;
			}
		...
		private sealed class QueryExpressionLambda : LambdaResolveResult
		{
			...
			public override string ToString()
			{
				return string.Format("[QueryExpressionLambda ({0}) => {1}]", string.Join(",", from p in parameters
					select p.Name), bodyExpression);
			}
		}
		...
		public void Scan(AstNode node)
		{
			...
			{
				if (resolverAfterDict.TryGetValue(node, out CSharpResolver value))
				{
					resolver = value;
				}
				return;
		...
		internal ResolveResult Resolve(AstNode node)
		{
			...
			if (!resolveResultCache.TryGetValue(node, out ResolveResult value))
			{
				cancellationToken.ThrowIfCancellationRequested();
				StoreCurrentState(node);
				CSharpResolver cSharpResolver = resolver;
		...
		{
			AnonymousFunctionConversion anonymousFunctionConversion = conversion as AnonymousFunctionConversion;
			if (anonymousFunctionConversion != null)
			{
		...
		public ResolveResult GetResolveResult(AstNode node)
		{
			...
			AstNode parent;
			CSharpResolver previouslyScannedContext = GetPreviouslyScannedContext(node, out parent);
		...
		{
			MergeUndecidedLambdas();
			if (resolverBeforeDict.TryGetValue(node, out CSharpResolver value))
			{
				return value;
			}
			AstNode parent;
			...
			ResetContext(previouslyScannedContext, delegate
			{
				navigator = new NodeListResolveVisitorNavigator(new AstNode[1]
				{
					node
				}, scanOnly: true);
				Scan(parent);
		...
		public ConversionWithTargetType GetConversionWithTargetType(Expression expr)
		{
			...
			if (conversionDict.TryGetValue(expr, out ConversionWithTargetType value))
			{
				return value;
			}
		...
		ResolveResult IAstVisitor<ResolveResult>.VisitParameterDeclaration(ParameterDeclaration parameterDeclaration)
		{
			...
			if (resolverEnabled)
			{
				...
				if (parameterDeclaration.Parent is DocumentationReference)
				{
					...
					if (parameterModifier == ParameterModifier.Ref || parameterModifier == ParameterModifier.Out)
					{
						type = new ByReferenceType(type);
					}
					return new LocalResolveResult(new DefaultParameter(type, name, null, default(DomRegion), null, parameterDeclaration.ParameterModifier == ParameterModifier.Ref, parameterDeclaration.ParameterModifier == ParameterModifier.Out, parameterDeclaration.ParameterModifier == ParameterModifier.Params));
				}
		...
		ResolveResult IAstVisitor<ResolveResult>.VisitTypeParameterDeclaration(TypeParameterDeclaration typeParameterDeclaration)
		{
			...
			if (resolverEnabled)
			{
				...
				IMethod method = resolver.CurrentMember as IMethod;
				if (method != null)
				{
		...
		ResolveResult IAstVisitor<ResolveResult>.VisitAnonymousTypeCreateExpression(AnonymousTypeCreateExpression anonymousTypeCreateExpression)
		{
			...
			foreach (Expression initializer2 in anonymousTypeCreateExpression.Initializers)
			{
				...
				string anonymousTypePropertyName = GetAnonymousTypePropertyName(initializer2, out resolveExpr);
				if (resolveExpr != null)
				{
			...
			for (int i = 0; i < list2.Count; i++)
			{
				...
				NamedExpression namedExpression = list2[i].Expression as NamedExpression;
				if (namedExpression != null)
				{
					StoreCurrentState(namedExpression);
					StoreResult(namedExpression, resolveResult2);
				}
				list4.Add(item);
			}
		...
		private void UnpackArrayInitializer(List<Expression> elementList, int[] sizes, ArrayInitializerExpression initializer, int dimension, bool resolveNestedInitializersToVoid)
		{
			...
			if (dimension + 1 < sizes.Length)
			{
				...
				{
					ArrayInitializerExpression arrayInitializerExpression = element as ArrayInitializerExpression;
		...
		ResolveResult IAstVisitor<ResolveResult>.VisitAsExpression(AsExpression asExpression)
		{
			if (resolverEnabled)
			{
				...
				return new CastResolveResult(ResolveType(asExpression.Type), input, Conversion.TryCast, resolver.CheckForOverflow);
			}
		...
		{
			OperatorResolveResult operatorResolveResult = rr as OperatorResolveResult;
			...
			{
				InvocationResolveResult invocationResolveResult = rr as InvocationResolveResult;
				if (invocationResolveResult != null && invocationResolveResult.Arguments.Count == 2)
				{
		...
		ResolveResult IAstVisitor<ResolveResult>.VisitConditionalExpression(ConditionalExpression conditionalExpression)
		{
			if (resolverEnabled)
			{
				...
				if (operatorResolveResult != null && operatorResolveResult.Operands.Count == 3)
				{
		...
		ResolveResult IAstVisitor<ResolveResult>.VisitDirectionExpression(DirectionExpression directionExpression)
		{
			if (resolverEnabled)
			{
				return new ByReferenceResolveResult(Resolve(directionExpression.Expression), directionExpression.FieldDirection == FieldDirection.Out);
			}
		...
		ResolveResult IAstVisitor<ResolveResult>.VisitIndexerExpression(IndexerExpression indexerExpression)
		{
			if (resolverEnabled || NeedsResolvingDueToNamedArguments(indexerExpression))
			{
				...
				ArrayAccessResolveResult arrayAccessResolveResult = resolveResult as ArrayAccessResolveResult;
		...
		private void HandleNamedExpression(NamedExpression namedExpression, List<ResolveResult> initializerStatements)
		{
			...
			else
			{
				...
				OperatorResolveResult operatorResolveResult = resolver.ResolveAssignment(AssignmentOperatorType.Assign, resolveResult, rhs) as OperatorResolveResult;
		...
		ResolveResult IAstVisitor<ResolveResult>.VisitObjectCreateExpression(ObjectCreateExpression objectCreateExpression)
		{
			...
			if (arguments.Length == 1 && resolveResult2.Type.Kind == TypeKind.Delegate)
			{
				...
				ConversionResolveResult conversionResolveResult = resolveResult2 as ConversionResolveResult;
				if (conversionResolveResult != null)
				{
					if (objectCreateExpression.Arguments.Count == 1)
					{
						ProcessConversionResult(objectCreateExpression.Arguments.Single(), conversionResolveResult);
					}
					return new CastResolveResult(conversionResolveResult);
				}
		...
		private void HandleObjectInitializer(ResolveResult initializedObject, ArrayInitializerExpression initializer, List<ResolveResult> initializerStatements)
		{
			...
			foreach (Expression element in initializer.Elements)
			{
				...
				if (arrayInitializerExpression != null)
				{
					...
					ResolveResult resolveResult = resolver.CreateMemberLookup().Lookup(initializedObject, "Add", EmptyList<IType>.Instance, isInvocation: true);
					...
					{
						CSharpInvocationResolveResult cSharpInvocationResolveResult = methodGroupResolveResult.PerformOverloadResolution(resolver.Compilation, array, null, allowExtensionMethods: false, allowExpandingParams: false, allowOptionalParameters: false, resolver.CheckForOverflow, resolver.conversions).CreateResolveResult(initializedObject);
		...
		ResolveResult IAstVisitor<ResolveResult>.VisitUnaryOperatorExpression(UnaryOperatorExpression unaryOperatorExpression)
		{
			if (resolverEnabled)
			{
				...
				OperatorResolveResult operatorResolveResult = resolveResult2 as OperatorResolveResult;
				if (operatorResolveResult != null && operatorResolveResult.Operands.Count == 1)
				{
					ProcessConversionResult(expression, operatorResolveResult.Operands[0] as ConversionResolveResult);
				}
				else
				{
					InvocationResolveResult invocationResolveResult = resolveResult2 as InvocationResolveResult;
					if (invocationResolveResult != null && invocationResolveResult.Arguments.Count == 1)
					{
		...
		ResolveResult IAstVisitor<ResolveResult>.VisitUndocumentedExpression(UndocumentedExpression undocumentedExpression)
		{
			...
			case UndocumentedExpressionType.ArgListAccess:
				type = ReflectionHelper.FindType(resolver.Compilation, typeof(RuntimeArgumentHandle));
				break;
			case UndocumentedExpressionType.ArgList:
				type = SpecialType.ArgList;
				break;
			case UndocumentedExpressionType.RefValue:
			{
				TypeReferenceExpression typeReferenceExpression = undocumentedExpression.Arguments.ElementAtOrDefault(1) as TypeReferenceExpression;
				type = ((typeReferenceExpression == null) ? SpecialType.UnknownType : ResolveType(typeReferenceExpression.Type));
				break;
			}
			case UndocumentedExpressionType.RefType:
				type = resolver.Compilation.FindType(KnownTypeCode.Type);
				break;
			case UndocumentedExpressionType.MakeRef:
				type = ReflectionHelper.FindType(resolver.Compilation, typeof(TypedReference));
				break;
			default:
		...
		private ResolveResult[] GetArguments(IEnumerable<Expression> argumentExpressions, out string[] argumentNames)
		{
			...
			{
				NamedArgumentExpression namedArgumentExpression = argumentExpression as NamedArgumentExpression;
				AstNode node;
		...
		{
			InvocationExpression invocationExpression = expr.Parent as InvocationExpression;
			if (invocationExpression != null && invocationExpression.Target == expr)
			{
		...
		private bool IsStaticResult(ResolveResult rr, ResolveResult invocationRR)
		{
			...
			return (((rr is MethodGroupResolveResult) ? invocationRR : rr) as MemberResolveResult)?.Member.IsStatic ?? false;
		}
		...
		ResolveResult IAstVisitor<ResolveResult>.VisitMemberReferenceExpression(MemberReferenceExpression memberReferenceExpression)
		{
			...
			if (identifierExpression != null && identifierExpression.TypeArguments.Count == 0)
			{
				...
				if (resolver.IsVariableReferenceWithSameType(resolveResult, identifierExpression.Identifier, out TypeResolveResult trr))
				{
		...
		private ExplicitlyTypedLambda HandleExplicitlyTypedLambda(AstNodeCollection<ParameterDeclaration> parameterDeclarations, AstNode body, bool isAnonymousMethod, bool hasParameterList, bool isAsync)
		{
			...
			foreach (ParameterDeclaration parameterDeclaration in parameterDeclarations)
			{
				...
				IParameter parameter = new DefaultParameter(type, parameterDeclaration.Name, null, MakeRegion(parameterDeclaration), null, parameterDeclaration.ParameterModifier == ParameterModifier.Ref, parameterDeclaration.ParameterModifier == ParameterModifier.Out);
		...
		private void ResolveParentForConversion(AstNode expression)
		{
			...
			if (parent != null && resolverBeforeDict.TryGetValue(parent, out CSharpResolver value))
			{
		...
		private void AnalyzeLambda(AstNode body, bool isAsync, out bool isValidAsVoidMethod, out bool isEndpointUnreachable, out IType inferredReturnType, out IList<Expression> returnExpressions, out IList<ResolveResult> returnValues)
		{
			...
			Expression expression = body as Expression;
			if (expression != null)
			{
				isValidAsVoidMethod = ExpressionPermittedAsStatement(expression);
				returnExpressions = new Expression[1]
				{
					expression
				};
				returnValues = new ResolveResult[1]
				{
					Resolve(expression)
				};
			...
			else
			{
				...
				else
				{
					...
					inferredReturnType = typeInference.GetBestCommonType(returnValues, out bool _);
		...
		ResolveResult IAstVisitor<ResolveResult>.VisitForeachStatement(ForeachStatement foreachStatement)
		{
			...
			{
				MethodGroupResolveResult methodGroupResolveResult = memberLookup.Lookup(resolveResult, "GetEnumerator", EmptyList<IType>.Instance, isInvocation: true) as MethodGroupResolveResult;
				if (methodGroupResolveResult != null)
				{
			...
			MethodGroupResolveResult methodGroupResolveResult2 = memberLookup.Lookup(new ResolveResult(enumeratorType), "MoveNext", EmptyList<IType>.Instance, isInvocation: false) as MethodGroupResolveResult;
			if (methodGroupResolveResult2 != null)
			{
				moveNextMethod = (methodGroupResolveResult2.PerformOverloadResolution(compilation, new ResolveResult[0], null, allowExtensionMethods: false, allowExpandingParams: false, allowOptionalParameters: false).GetBestCandidateWithSubstitutedTypeArguments() as IMethod);
			}
		...
		{
			ICompilation compilation = resolver.Compilation;
			elementType = GetElementTypeFromIEnumerable(expression.Type, compilation, allowIEnumerator: false, out bool? isGeneric);
			if (isGeneric == true)
			{
				ITypeDefinition definition = compilation.FindType(KnownTypeCode.IEnumerableOfT).GetDefinition();
				if (definition != null)
				{
					collectionType = new ParameterizedType(definition, new IType[1]
					{
						elementType
					});
				}
				...
				if (definition2 != null)
				{
					enumeratorType = new ParameterizedType(definition2, new IType[1]
					{
						elementType
					});
				}
		...
		{
			bool flag = (variableDeclarationStatement.Modifiers & Modifiers.Const) != Modifiers.None;
		...
		ResolveResult IAstVisitor<ResolveResult>.VisitYieldReturnStatement(YieldReturnStatement yieldStatement)
		{
			...
			{
				bool? isGeneric;
				IType elementTypeFromIEnumerable = GetElementTypeFromIEnumerable(resolver.CurrentMember.ReturnType, resolver.Compilation, allowIEnumerator: true, out isGeneric);
		...
		private static IType GetElementTypeFromIEnumerable(IType collectionType, ICompilation compilation, bool allowIEnumerator, out bool? isGeneric)
		{
			...
			foreach (IType allBaseType in collectionType.GetAllBaseTypes())
			{
				...
				if (definition != null)
				{
					...
					if (knownTypeCode == KnownTypeCode.IEnumerableOfT || (allowIEnumerator && knownTypeCode == KnownTypeCode.IEnumeratorOfT))
					{
						ParameterizedType parameterizedType = allBaseType as ParameterizedType;
		...
		ResolveResult IAstVisitor<ResolveResult>.VisitAttribute(Attribute attribute)
		{
			...
			IEnumerable<Expression> enumerable = from a in attribute.Arguments
				where !(a is NamedExpression)
				select a;
			...
			resolver = resolver.PopObjectInitializer();
			string[] argumentNames;
			ResolveResult[] arguments = GetArguments(enumerable, out argumentNames);
		...
		ResolveResult IAstVisitor<ResolveResult>.VisitPrimitiveType(PrimitiveType primitiveType)
		{
			...
			return new TypeResolveResult(resolver.Compilation.FindType(knownTypeCode));
		}
		...
		{
			bool? isGeneric;
			IType elementTypeFromIEnumerable = GetElementTypeFromIEnumerable(type, resolver.Compilation, allowIEnumerator: false, out isGeneric);
			if (elementTypeFromIEnumerable.Kind == TypeKind.Unknown)
			{
				...
				CSharpInvocationResolveResult cSharpInvocationResolveResult = resolver.ResolveInvocation(target, arguments) as CSharpInvocationResolveResult;
				if (cSharpInvocationResolveResult != null && cSharpInvocationResolveResult.Arguments.Count == 2)
				{
		...
		ResolveResult IAstVisitor<ResolveResult>.VisitQueryFromClause(QueryFromClause queryFromClause)
		{
			...
			else
			{
				...
				ResolveResult target = resolver.ResolveMemberAccess(resolveResult2, "Cast", new IType[1]
				{
					variable.Type
			...
			if (currentQueryResult != null)
			{
				...
				ResolveResult bodyExpression = (querySelectClause == null) ? MakeTransparentIdentifierResolveResult() : Resolve(querySelectClause.Expression);
		...
		private ResolveResult ResolveGroupJoin(QueryJoinClause queryJoinClause, ResolveResult inResult, ResolveResult onResult, ResolveResult equalsResult)
		{
			...
			if (querySelectClause != null)
			{
				...
				lambdaResolveResult = new ImplicitlyTypedLambda(querySelectClause, parameters, this);
			}
			else
			{
			...
			if (lambdaResolveResult is ImplicitlyTypedLambda)
			{
				...
				if (invocationResolveResult != null && invocationResolveResult.Arguments.Count > 0)
				{
					ConversionResolveResult conversionResolveResult = invocationResolveResult.Arguments[invocationResolveResult.Arguments.Count - 1] as ConversionResolveResult;
					if (conversionResolveResult != null)
					{
			...
			else
			{
				...
				if (invocationResolveResult != null && invocationResolveResult.Arguments.Count > 0)
				{
					ConversionResolveResult conversionResolveResult2 = invocationResolveResult.Arguments[invocationResolveResult.Arguments.Count - 1] as ConversionResolveResult;
					if (conversionResolveResult2 != null && conversionResolveResult2.Conversion is QueryExpressionLambdaConversion)
					{
		...
		ResolveResult IAstVisitor<ResolveResult>.VisitQuerySelectClause(QuerySelectClause querySelectClause)
		{
			...
			if (singleRangeVariable != null)
			{
				IdentifierExpression identifierExpression = ParenthesizedExpression.UnpackParenthesizedExpression(querySelectClause.Expression) as IdentifierExpression;
				if (identifierExpression != null && identifierExpression.Identifier == singleRangeVariable && !identifierExpression.TypeArguments.Any() && queryExpression.Clauses.Count > 2)
				{
					Scan(querySelectClause.Expression);
					return WrapResult(currentQueryResult);
				}
		...
		private string GetSingleRangeVariable(QueryExpression query)
		{
			...
			QueryFromClause queryFromClause = query.Clauses.FirstOrDefault() as QueryFromClause;
			if (queryFromClause != null)
			{
				return queryFromClause.Identifier;
			}
			return (query.Clauses.FirstOrDefault() as QueryContinuationClause)?.Identifier;
		}
		...
		ResolveResult IAstVisitor<ResolveResult>.VisitQueryOrdering(QueryOrdering queryOrdering)
		{
			...
			QueryOrderClause queryOrderClause = queryOrdering.Parent as QueryOrderClause;
			string text = (queryOrderClause == null || queryOrderClause.Orderings.FirstOrDefault() == queryOrdering) ? "OrderBy" : "ThenBy";
		...
		ResolveResult IAstVisitor<ResolveResult>.VisitDocumentationReference(DocumentationReference documentationReference)
		{
			...
			{
				string memberName2 = documentationReference.MemberName;
				ResolveResult resolveResult;
				if (documentationReference.DeclaringType.IsNull)
				{
					resolveResult = resolver.LookupSimpleNameOrTypeName(memberName2, typeArguments, NameLookupMode.Expression);
				}
				else
				{
					ResolveResult target = Resolve(documentationReference.DeclaringType);
					resolveResult = resolver.ResolveMemberAccess(target, memberName2, typeArguments);
				}
				if (resolveResult.IsError)
				{
					return resolveResult;
				...
				{
					IEnumerable<IParameterizedMember> methods = ((MethodGroupResolveResult)resolveResult).MethodsGroupedByDeclaringType.Reverse().SelectMany((MethodListWithDeclaringType ml) => from m in ml
						select (IParameterizedMember)m.MemberDefinition);
			...
			{
				IEnumerable<IProperty> methods2 = from p in typeDefinition.Properties
					where p.IsIndexer && !p.IsExplicitInterfaceImplementation
					select p;
			...
			if (documentationReference.SymbolKind == SymbolKind.Operator)
			{
				...
				string memberName = OperatorDeclaration.GetName(operatorType);
				IEnumerable<IMethod> enumerable = from m in typeDefinition.Methods
					where m.IsOperator && m.Name == memberName
					select m;
		...
		{
			LocalResolveResult localResolveResult = Resolve(p) as LocalResolveResult;
