...
namespace Mono.CompilerServices.SymbolWriter
{
	public class MonoSymbolFile : IDisposable
	{
		...
		public SourceFileEntry GetSourceFile(int index)
		{
			...
			lock (this)
			{
				if (source_file_hash.TryGetValue(index, out SourceFileEntry value))
				{
					return value;
				}
		...
		public CompileUnitEntry GetCompileUnit(int index)
		{
			...
			lock (this)
			{
				if (compile_unit_hash.TryGetValue(index, out CompileUnitEntry value))
				{
					return value;
				}
				long position = reader.BaseStream.Position;
		...
		public MethodEntry GetMethodByToken(int token)
		{
			...
			{
				read_methods();
				method_token_hash.TryGetValue(token, out MethodEntry value);
				return value;
			}
		}
		...
		public AnonymousScopeEntry GetAnonymousScope(int id)
		{
			...
			lock (this)
			{
				if (anonymous_scopes != null)
				{
					anonymous_scopes.TryGetValue(id, out AnonymousScopeEntry value);
					return value;
				}
				anonymous_scopes = new Dictionary<int, AnonymousScopeEntry>();
