...
namespace Mono.CompilerServices.SymbolWriter
{
	public class MethodEntry : IComparable
	{
		...
		public enum Flags
		{
			LocalNamesAmbiguous = 0x1,
			ColumnsInfoIncluded = 0x2,
			EndInfoIncluded = 0x4
		}
		...
		internal MethodEntry(MonoSymbolFile file, CompileUnitEntry comp_unit, int token, ScopeVariable[] scope_vars, LocalVariableEntry[] locals, LineNumberEntry[] lines, CodeBlockEntry[] code_blocks, string real_name, Flags flags, int namespace_id)
		{
			...
			while (true)
			{
				...
				{
					LocalVariableEntry localVariableEntry = locals[num2];
		...
		internal void WriteData(MonoSymbolFile file, MyBinaryWriter bw)
		{
			...
			lnt.Write(file, bw, (flags & Flags.ColumnsInfoIncluded) != (Flags)0, (flags & Flags.EndInfoIncluded) != (Flags)0);
		...
		public LineNumberTable GetLineNumberTable()
		{
			lock (SymbolFile)
			{
				...
				lnt = LineNumberTable.Read(SymbolFile, binaryReader, (flags & Flags.ColumnsInfoIncluded) != (Flags)0, (flags & Flags.EndInfoIncluded) != (Flags)0);
