...
namespace ICSharpCode.NRefactory.CSharp
{
	public class CSharpAmbience : IAmbience
	{
		...
		public string ConvertSymbol(ISymbol symbol)
		{
			...
			ConvertSymbol(symbol, new TextWriterTokenWriter(stringWriter), FormattingOptionsFactory.CreateMono());
		...
		public void ConvertSymbol(ISymbol symbol, TokenWriter writer, CSharpFormattingOptions formattingPolicy)
		{
			...
			AstNode astNode = CreateAstBuilder().ConvertSymbol(symbol);
		...
		private TypeSystemAstBuilder CreateAstBuilder()
		{
			...
			{
				AddTypeReferenceAnnotations = true,
		...
		{
			EntityDeclaration entityDeclaration = CreateAstBuilder().ConvertEntity(typeDef);
			...
			{
				new CSharpOutputVisitor(writer, formattingPolicy).WriteTypeParameters(entityDeclaration.GetChildrenByRole(Roles.TypeParameter));
			}
		}

		private void WriteMemberDeclarationName(IMember member, TokenWriter writer, CSharpFormattingOptions formattingPolicy)
		{
			EntityDeclaration entityDeclaration = CreateAstBuilder().ConvertEntity(member);
		...
		{
			return CreateAstBuilder().ConvertVariable(v).ToString().TrimEnd(';', '\r', '\n', '\u2028');
		}
		...
		public string ConvertType(IType type)
		{
			...
			return typeSystemAstBuilder.ConvertType(type).ToString();
		}
		...
		public void ConvertType(IType type, TokenWriter writer, CSharpFormattingOptions formattingPolicy)
		{
			TypeSystemAstBuilder typeSystemAstBuilder = CreateAstBuilder();
			typeSystemAstBuilder.AlwaysUseShortTypeNames = ((ConversionFlags & ConversionFlags.UseFullyQualifiedEntityNames) != ConversionFlags.UseFullyQualifiedEntityNames);
			typeSystemAstBuilder.ConvertType(type).AcceptVisitor(new CSharpOutputVisitor(writer, formattingPolicy));
		}
