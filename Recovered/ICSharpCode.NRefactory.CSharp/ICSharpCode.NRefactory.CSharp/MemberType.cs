...
namespace ICSharpCode.NRefactory.CSharp
{
	public class MemberType : AstType
	{
		...
		{
			MemberType memberType = other as MemberType;
			if (memberType != null && IsDoubleColon == memberType.IsDoubleColon && AstNode.MatchString(MemberName, memberType.MemberName) && Target.DoMatch(memberType.Target, match))
			{
		...
		public override ITypeReference ToTypeReference(NameLookupMode lookupMode, InterningProvider interningProvider = null)
		{
			...
			TypeOrNamespaceReference typeOrNamespaceReference;
			if (IsDoubleColon)
			{
				SimpleType simpleType = Target as SimpleType;
				typeOrNamespaceReference = ((simpleType == null) ? null : interningProvider.Intern(new AliasNamespaceReference(interningProvider.Intern(simpleType.Identifier))));
			}
			else
			{
				typeOrNamespaceReference = (Target.ToTypeReference(lookupMode, interningProvider) as TypeOrNamespaceReference);
			}
