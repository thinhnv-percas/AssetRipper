...
namespace ICSharpCode.NRefactory.CSharp.TypeSystem
{
	public class ResolvedUsingScope
	{
		...
		public INamespace Namespace
		{
			get
			{
				INamespace @namespace = LazyInit.VolatileRead(ref this.@namespace);
				if (@namespace != null)
				{
					return @namespace;
				}
				...
				{
					@namespace = parentContext.CurrentUsingScope.Namespace.GetChildNamespace(usingScope.ShortNamespaceName);
					if (@namespace == null)
					{
						@namespace = new DummyNamespace(parentContext.CurrentUsingScope.Namespace, usingScope.ShortNamespaceName);
					}
				}
				else
				{
					@namespace = parentContext.Compilation.RootNamespace;
				}
				return LazyInit.GetOrSet(ref this.@namespace, @namespace);
			}
		}
		...
		public IList<INamespace> Usings
		{
			get
			{
				...
				{
					INamespace @namespace = @using.ResolveNamespace(resolver);
