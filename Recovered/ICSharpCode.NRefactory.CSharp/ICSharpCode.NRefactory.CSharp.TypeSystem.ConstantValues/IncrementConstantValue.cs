...
namespace ICSharpCode.NRefactory.CSharp.TypeSystem.ConstantValues
{
	...
	public sealed class IncrementConstantValue : IConstantValue, ISupportsInterning
	{
		...
		public IncrementConstantValue(IConstantValue baseValue, int incrementAmount = 1)
		{
			...
			IncrementConstantValue incrementConstantValue = baseValue as IncrementConstantValue;
			if (incrementConstantValue != null)
			{
		...
		public ResolveResult Resolve(ITypeResolveContext context)
		{
			...
			if (resolveResult.IsCompileTimeConstant && resolveResult.ConstantValue != null)
			{
				...
				TypeCode typeCode = Type.GetTypeCode(constantValue.GetType());
		...
		{
			IncrementConstantValue incrementConstantValue = other as IncrementConstantValue;
			if (incrementConstantValue != null && baseValue == incrementConstantValue.baseValue)
			{
