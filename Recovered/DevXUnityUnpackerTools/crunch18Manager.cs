...
internal class crunch18Manager
{
	internal const string txt = "crunch_2018";

	internal static string path
	{
		get
		{
			string _0020 = FileManager.FormatPath(DevXSystemInfo.StreamingAssets, "Crunch2018", "Win");
			string text;
			if (Environment.Is64BitProcess)
			{
				text = FileManager.FormatPath(_0020, "crunch_2018_x64.exe");
				if (File.Exists(text))
				{
					return text;
				}
			}
			text = FileManager.FormatPath(_0020, "crunch_2018_x86.exe");
			if (File.Exists(text))
			{
				return text;
			}
			text = FileManager.FormatPath(_0020, "crunch_2018_x32.exe");
			if (File.Exists(text))
			{
				return text;
			}
			text = FileManager.FormatPath(_0020, "crunch_2018.exe");
			if (File.Exists(text))
			{
				return text;
			}
	...
	public static void Decode(byte[] inputBytes, out byte[] pngBytes)
	{
		...
		try
		{
			...
			{
				ConsoleManager.Write("crunch_2018 no find");
			}
			else
			{
				pngBytes = null;
