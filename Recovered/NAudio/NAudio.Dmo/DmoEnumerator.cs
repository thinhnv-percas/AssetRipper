...
namespace NAudio.Dmo
{
	public class DmoEnumerator
	{
		...
		{
			Marshal.ThrowExceptionForHR(DmoInterop.DMOEnum(ref category, DmoEnumFlags.None, 0, null, 0, null, out IEnumDmo enumDmo));
			int itemsFetched;
			do
			{
				enumDmo.Next(1, out Guid clsid, out IntPtr name, out itemsFetched);
				if (itemsFetched == 1)
				{
