...
namespace NAudio.Wave
{
	public class Id3v2Tag
	{
		...
		private static byte[] CreateId3v2Frame(string key, string value)
		{
			...
			byte[] array4 = (!(key == "COMM")) ? ByteArrayExtensions.Concat(new byte[1]
			{
				1
			}, array, Encoding.Unicode.GetBytes(value)) : ByteArrayExtensions.Concat(new byte[1]
			{
				1
			}, array2, array3, array, Encoding.Unicode.GetBytes(value));
		...
		private static byte[] CreateId3v2TagHeader(IEnumerable<byte[]> frames)
		{
			...
			return ByteArrayExtensions.Concat(Encoding.UTF8.GetBytes("ID3"), new byte[2]
			{
				3,
				0
			}, new byte[1], GetId3TagHeaderSize(num));
		}
		...
		private Id3v2Tag(Stream input)
		{
			...
			if (array.Length >= 3 && array[0] == 73 && array[1] == 68 && array[2] == 51)
			{
				if ((array[5] & 0x40) == 64)
				{
					...
					int num2 = array2[0] * 2097152 + array2[1] * 16384 + array2[2] * 128;
					byte b = array2[3];
				}
