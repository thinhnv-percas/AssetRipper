...
namespace NAudio.Wave
{
	public class AiffFileReader : WaveStream
	{
		...
		public static void ReadAiffHeader(Stream stream, out WaveFormat format, out long dataChunkPosition, out int dataChunkLength, List<AiffChunk> chunks)
		{
			...
			{
				AiffChunk aiffChunk = ReadChunkHeader(binaryReader);
				if (aiffChunk.ChunkName == "\0\0\0\0" || binaryReader.BaseStream.Position + aiffChunk.ChunkLength > binaryReader.BaseStream.Length)
				{
					break;
				}
				if (aiffChunk.ChunkName == "COMM")
				{
					...
					if (aiffChunk.ChunkLength > 18 && a == "AIFC")
					{
						if (new string(binaryReader.ReadChars(4)).ToLower() != "none")
						{
							throw new FormatException("Compressed AIFC is not supported.");
						}
						binaryReader.ReadBytes((int)(aiffChunk.ChunkLength - 22));
					}
					else
					{
						binaryReader.ReadBytes((int)(aiffChunk.ChunkLength - 18));
					}
				}
				else if (aiffChunk.ChunkName == "SSND")
				{
					...
					ConvertInt(binaryReader.ReadBytes(4));
					dataChunkPosition = aiffChunk.ChunkStart + 16 + num2;
					dataChunkLength = (int)(aiffChunk.ChunkLength - 8);
					binaryReader.BaseStream.Position += aiffChunk.ChunkLength - 8;
				}
				else
				{
					chunks?.Add(aiffChunk);
					binaryReader.BaseStream.Position += aiffChunk.ChunkLength;
				}
			}
			if (format == null)
			{
