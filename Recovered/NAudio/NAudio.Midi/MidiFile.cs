...
namespace NAudio.Midi
{
	public class MidiFile
	{
			...
			try
			{
				...
				while (true)
				{
					...
					while (binaryReader.BaseStream.Position < position + num)
					{
						...
						{
							throw new FormatException($"End Track event was not the last MIDI event on track {num4}");
						}
					}
		...
		{
			return ((uint)((int)i & -16777216) >> 24) | ((i & 0xFF0000) >> 8) | ((i & 0xFF00) << 8) | ((i & 0xFF) << 24);
		}
