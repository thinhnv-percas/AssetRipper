	...
	public static class IdStringProvider
	{
		...
		private static ITypeReference ParseTypeName(string typeName, ref int pos)
		{
			...
			else
			{
				...
				typeReference = new GetPotentiallyNestedClassTypeReference(ReadTypeName(typeName, ref pos, allowDottedName: true, out int typeParameterCount, list), typeParameterCount);
