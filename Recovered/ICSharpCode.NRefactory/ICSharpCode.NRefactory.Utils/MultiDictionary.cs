...
namespace ICSharpCode.NRefactory.Utils
{
	public class MultiDictionary<TKey, TValue> : ILookup<TKey, TValue>, IEnumerable<IGrouping<TKey, TValue>>, IEnumerable
	{
		...
		public IList<TValue> this[TKey key]
		{
			get
			{
				if (dict.TryGetValue(key, out List<TValue> value))
				{
					return value;
				}
		...
		public IEnumerable<TValue> Values => dict.Values.SelectMany((List<TValue> list) => list);
		...
		{
			if (!dict.TryGetValue(key, out List<TValue> value2))
			{
		...
		{
			if (dict.TryGetValue(key, out List<TValue> value2) && value2.Remove(value))
			{
				if (value2.Count == 0)
				{
					dict.Remove(key);
