	...
	public sealed class DefaultUnresolvedAttribute : AbstractFreezable, IUnresolvedAttribute, IFreezable, ISupportsInterning
	{
		private sealed class DefaultResolvedAttribute : IAttribute, ICompilationProvider
		{
			...
			private IMethod ResolveConstructor()
			{
				...
				foreach (IMethod constructor2 in attributeType.GetConstructors((IUnresolvedMethod m) => m.Parameters.Count == parameterTypes.Count))
				{
					bool flag = true;
					for (int i = 0; i < parameterTypes.Count; i++)
					{
						if (!constructor2.Parameters[i].Type.Equals(parameterTypes[i]))
						{
							flag = false;
							break;
						}
		...
		bool ISupportsInterning.EqualsForInterning(ISupportsInterning other)
		{
			...
			if (defaultUnresolvedAttribute != null && attributeType == defaultUnresolvedAttribute.attributeType && ListEquals(constructorParameterTypes, defaultUnresolvedAttribute.constructorParameterTypes) && ListEquals(positionalArguments, defaultUnresolvedAttribute.positionalArguments))
			{
