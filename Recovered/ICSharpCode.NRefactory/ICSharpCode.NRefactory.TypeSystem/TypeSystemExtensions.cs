	...
	public static class TypeSystemExtensions
	{
		...
		public static ISymbol Import(this ICompilation compilation, ISymbol symbol)
		{
			...
			case SymbolKind.Parameter:
			{
				...
				return new DefaultParameter(compilation.Import(parameter.Type), parameter.Name, null, parameter.Region, null, parameter.IsRef, parameter.IsOut, parameter.IsParams);
			}
		...
		private static IEnumerable<IAttribute> GetAttributes(IEntity entity, Predicate<IType> attributeTypePredicate, bool inherit)
		{
			...
			ITypeDefinition typeDefinition = entity as ITypeDefinition;
			if (typeDefinition != null)
			{
				foreach (IType item in typeDefinition.GetNonInterfaceBaseTypes().Reverse())
				{
			...
			IMember member2 = entity as IMember;
			if (member2 != null)
			{
				...
				do
				{
					member2 = member2.MemberDefinition;
					if (visitedMembers.Add(member2))
					{
						foreach (IAttribute attribute3 in member2.Attributes)
						{
						...
						{
							member2 = (baseMember = InheritanceHelper.GetBaseMember(member2));
							continue;
						}
						break;
					}
