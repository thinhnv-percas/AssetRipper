...
public sealed class PackageRemappingTests
{
	...
	/// <summary>
	/// Nothing in an export says which folder a package was ripped into, so it is found by its shape:
	/// the relative paths the official package holds its assets at.
	/// </summary>
	[Test]
	public void APackageIsLocatedByTheShapeUnderneathIt()
	{
		using TemporaryDirectory directory = new();
		string assets = System.IO.Path.Combine(directory.Path, "Assets");
		Write(assets, "TextMesh Pro/Shaders/TMP_SDF.shader");
		Write(assets, "TextMesh Pro/Shaders/TMP_Bitmap.shader");
		Write(assets, "TextMesh Pro/Resources/LiberationSans.asset");
		Write(assets, "Unrelated/Shaders/TMP_SDF.shader");

		List<AssetIdentity> official =
		[
			new("Shaders/TMP_SDF.shader", "1"),
			new("Shaders/TMP_Bitmap.shader", "2"),
			new("Resources/LiberationSans.asset", "3"),
		];

		string? located = RippedPackageLocator.Locate(assets, official);

		Assert.That(located, Is.EqualTo(System.IO.Path.Combine(assets, "TextMesh Pro").Replace('\\', '/')).IgnoreCase);
	}

	/// <summary>
	/// A folder that happens to hold one file of the same name is a coincidence, not the package, and
	/// acting on it would repoint references at the wrong assets.
	/// </summary>
	[Test]
	public void OneCoincidenceIsNotEnoughToPlaceAPackage()
	{
		using TemporaryDirectory directory = new();
		string assets = System.IO.Path.Combine(directory.Path, "Assets");
		Write(assets, "Unrelated/Shaders/TMP_SDF.shader");
		...
		[
			...
			new("Shaders/TMP_Bitmap.shader", "2"),
			new("Resources/LiberationSans.asset", "3"),
			new("Fonts/LiberationSans.ttf", "4"),
		];

		Assert.That(RippedPackageLocator.Locate(assets, official), Is.Null);
	}

	/// <summary>
	/// A tail has to land on a folder boundary, or Shaders/TMP.shader would be matched by anything
	/// ending in those characters.
	/// </summary>
	[Test]
	public void AMatchHasToFallOnAFolderBoundary()
	{
		using TemporaryDirectory directory = new();
		string assets = System.IO.Path.Combine(directory.Path, "Assets");
		Write(assets, "Pack/OtherShaders/A.shader");
		Write(assets, "Pack/OtherShaders/B.shader");
		Write(assets, "Pack/OtherShaders/C.shader");

		List<AssetIdentity> official =
		[
			new("Shaders/A.shader", "1"),
			new("Shaders/B.shader", "2"),
			new("Shaders/C.shader", "3"),
		];
