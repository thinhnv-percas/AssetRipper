...
public class MultipartContent : HttpContent, IEnumerable<HttpContent>, IEnumerable
{
	...
	private static void ValidateBoundary(string boundary)
	{
		...
		{
			throw new ArgumentOutOfRangeException("boundary", boundary, string.Format(CultureInfo.InvariantCulture, System.SR.net_http_content_field_too_long, 70));
		}
		if (boundary.EndsWith(" ", StringComparison.Ordinal))
		{
			throw new ArgumentException(string.Format(CultureInfo.InvariantCulture, System.SR.net_http_headers_invalid_value, boundary), "boundary");
		}
	...
	protected override async Task<Stream> CreateContentReadStreamAsync()
	{
		...
		try
		{
			...
			for (int contentIndex = 0; contentIndex < _nestedContent.Count; contentIndex++)
			{
				...
				Stream readStream = (await httpContent.ReadAsStreamAsync().ConfigureAwait(continueOnCapturedContext: false)) ?? new MemoryStream();
