...
public class ContentRangeHeaderValue : ICloneable
{
	...
	public override bool Equals(object obj)
	{
		...
		if (_from == contentRangeHeaderValue._from)
		{
			long? to = _to;
			long? to2 = contentRangeHeaderValue._to;
			if (to.GetValueOrDefault() == to2.GetValueOrDefault() && to.HasValue == to2.HasValue && _length == contentRangeHeaderValue._length)
			{
	...
	private static bool TryCreateContentRange(string input, string unit, int fromStartIndex, int fromLength, int toStartIndex, int toLength, int lengthStartIndex, int lengthLength, out object parsedValue)
	{
		...
		long result2 = 0L;
		if (toLength > 0 && !HeaderUtilities.TryParseInt64(input.Substring(toStartIndex, toLength), out result2))
		{
			return false;
		}
		...
		long result3 = 0L;
		if (lengthLength > 0 && !HeaderUtilities.TryParseInt64(input.Substring(lengthStartIndex, lengthLength), out result3))
		{
			return false;
		}
