...
public class ContentDispositionHeaderValue : ICloneable
{
	...
	private static void CheckDispositionTypeFormat(string dispositionType, string parameterName)
	{
		...
		{
			throw new FormatException(string.Format(CultureInfo.InvariantCulture, System.SR.net_http_headers_invalid_value, dispositionType));
		}
	}
	...
	private void SetName(string parameter, string value)
	{
		...
		empty = ((!parameter.EndsWith("*", StringComparison.Ordinal)) ? EncodeAndQuoteMime(value) : Encode5987(value));
		if (nameValueHeaderValue != null)
		{
	...
	private string EncodeAndQuoteMime(string input)
	{
		...
		{
			throw new ArgumentException(string.Format(CultureInfo.InvariantCulture, System.SR.net_http_headers_invalid_value, input));
		}
	...
	private string Encode5987(string input)
	{
		...
		for (int i = 0; i < input.Length; i++)
		{
			...
			if (c > '\u007f')
			{
				...
				{
					stringBuilder.Append(UriShim.HexEscape((char)character));
				}
			}
			else if (!HttpRuleParser.IsTokenChar(c) || c == '*' || c == '\'' || c == '%')
			{
				stringBuilder.Append(UriShim.HexEscape(c));
			}
			else
			{
	...
	private bool TryDecode5987(string input, out string output)
	{
		...
		try
		{
			...
			{
				if (UriShim.IsHexEncoding(text2, i))
				{
					array[num3++] = (byte)UriShim.HexUnescape(text2, ref i);
					i--;
					continue;
				}
				if (num3 > 0)
