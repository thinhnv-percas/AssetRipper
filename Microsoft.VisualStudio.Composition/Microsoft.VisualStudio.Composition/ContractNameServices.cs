...
internal static class ContractNameServices
{
	...
	internal static string GetTypeIdentity(Type type, bool formatGenericName)
	{
		...
		if (!TypeIdentityCache.TryGetValue(type, out value))
		{
			...
			{
				value = GetTypeIdentityFromMethod(type.GetTypeInfo().GetDeclaredMethod("Invoke"));
			}
			...
			Assumes.True(!string.IsNullOrEmpty(value));
	...
	private static void WriteType(StringBuilder typeName, Type type, bool formatGenericName)
	{
		if (type.GetTypeInfo().IsGenericType)
		{
			...
			WriteGenericType(typeName, type, type.GetTypeInfo().IsGenericTypeDefinition, queue, formatGenericName);
			Assumes.True(queue.Count == 0, "Expecting genericTypeArguments queue to be empty.");
		}
		else
		{
