internal sealed class ThisAssembly
{
	internal const string AssemblyVersion = "15.5.0.0";

	internal const string AssemblyFileVersion = "15.5.23.32629";

	internal const string AssemblyInformationalVersion = "15.5.23+g757fe180d9.RR";

	internal const string AssemblyName = "Microsoft.VisualStudio.Composition";

	internal const string AssemblyTitle = "Microsoft.VisualStudio.Composition";

	internal const string AssemblyConfiguration = "Release";

	internal const string PublicKey = "";

	internal const string PublicKeyToken = "b03f5f7f11d50a3a";

	internal const string RootNamespace = "Microsoft.VisualStudio.Composition";

	private ThisAssembly()
	{
	}
}
