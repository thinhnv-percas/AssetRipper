...
public class FormatItem : FormatStringSegmentBase
{
	...
	private bool FieldsEquals(FormatItem other)
	{
		if (Index == other.Index)
		{
			...
			int? alignment2 = other.Alignment;
			if (alignment.GetValueOrDefault() == alignment2.GetValueOrDefault() && alignment.HasValue == alignment2.HasValue && FormatString == other.FormatString && base.StartLocation == other.StartLocation)
			{
	...
	{
		int num = 23;
		num = num * 37 + Index.GetHashCode();
		num = num * 37 + Alignment.GetHashCode();
		num = num * 37 + FormatString.GetHashCode();
