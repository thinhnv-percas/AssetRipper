...
public sealed class ParameterizedType : IType, INamedElement, IEquatable<IType>, ICompilationProvider
{
	...
	public IEnumerable<IType> DirectBaseTypes
	{
		get
		{
			...
			return genericType.DirectBaseTypes.Select((IType t) => t.AcceptVisitor(substitution));
		}
	}
	...
	public ParameterizedType(ITypeDefinition genericType, IEnumerable<IType> typeArguments)
	{
		...
		for (int i = 0; i < this.typeArguments.Length; i++)
		{
			...
			if (this.typeArguments[i] is ICompilationProvider compilationProvider && compilationProvider.Compilation != genericType.Compilation)
			{
	...
	{
		return new ParameterizedTypeReference(genericType.ToTypeReference(), typeArguments.Select((IType t) => t.ToTypeReference()));
	}
