...
public sealed class DefaultUnresolvedParameter : IUnresolvedParameter, IFreezable, ISupportsInterning
{
	private sealed class ResolvedParameterWithDefaultValue : IParameter, IVariable, ISymbol
	{
		...
		public ISymbolReference ToReference()
		{
			if (Owner == null)
			{
				return new ParameterReference(Type.ToTypeReference(), Name, Region, IsRef, IsOut, IsParams, isOptional: true, ConstantValue, IsIn);
			}
	...
	{
		if (other is DefaultUnresolvedParameter defaultUnresolvedParameter && type == defaultUnresolvedParameter.type && name == defaultUnresolvedParameter.name && defaultValue == defaultUnresolvedParameter.defaultValue && region == defaultUnresolvedParameter.region && (flags & -2) == (defaultUnresolvedParameter.flags & -2))
		{
	...
	public IParameter CreateResolvedParameter(ITypeResolveContext context)
	{
		...
		return new DefaultParameter(type.Resolve(context), name, owner, region, list, IsRef, IsOut, IsParams, isOptional, null, IsIn);
	}
}
