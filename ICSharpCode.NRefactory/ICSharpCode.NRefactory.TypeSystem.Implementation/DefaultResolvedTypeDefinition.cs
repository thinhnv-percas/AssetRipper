...
public class DefaultResolvedTypeDefinition : ITypeDefinition, IType, INamedElement, IEquatable<IType>, IEntity, ISymbol, ICompilationProvider, IHasAccessibility
{
	...
	public IList<IAttribute> Attributes
	{
		get
		{
			...
			foreach (IUnresolvedTypeDefinition unresolvedTypeDefinition in array)
			{
				...
				foreach (IUnresolvedAttribute attribute in unresolvedTypeDefinition.Attributes)
				{
	...
	public IEnumerable<IProperty> Properties
	{
		get
		{
			...
			for (int i = 0; i < members.unresolvedMembers.Length; i++)
			{
				...
				if (symbolKind - 3 <= SymbolKind.TypeDefinition)
				{
	...
	public virtual DocumentationComment Documentation
	{
		get
		{
			...
			foreach (IUnresolvedTypeDefinition unresolvedTypeDefinition in array)
			{
				if (unresolvedTypeDefinition.UnresolvedFile is IUnresolvedDocumentationProvider unresolvedDocumentationProvider)
				{
	...
	private MemberList GetMemberList()
	{
		...
		foreach (IUnresolvedTypeDefinition unresolvedTypeDefinition in array)
		{
			ITypeResolveContext typeResolveContext = unresolvedTypeDefinition.CreateResolveContext(parentContext);
			...
			{
				if (member is IUnresolvedMethod { IsPartial: not false } unresolvedMethod)
				{
					if (list3 == null)
					{
						list3 = new List<PartialMethodInfo>();
					}
					PartialMethodInfo partialMethodInfo = new PartialMethodInfo(unresolvedMethod, typeResolveContext2);
	...
	private IType CalculateEnumUnderlyingType()
	{
		...
		foreach (IUnresolvedTypeDefinition unresolvedTypeDefinition in array)
		{
			ITypeResolveContext context = unresolvedTypeDefinition.CreateResolveContext(parentContext).WithCurrentTypeDefinition(this);
			foreach (ITypeReference baseType in unresolvedTypeDefinition.BaseTypes)
			{
	...
	private IEnumerable<IMethod> GetFilteredMethods(Predicate<IUnresolvedMethod> filter)
	{
		...
		for (int i = 0; i < members.unresolvedMembers.Length; i++)
		{
			if (members.unresolvedMembers[i] is IUnresolvedMethod obj && (filter == null || filter(obj)))
			{
				yield return (IMethod)members[i];
			}
		}
	...
	private IEnumerable<TResolved> GetFilteredNonMethods<TUnresolved, TResolved>(Predicate<TUnresolved> filter) where TUnresolved : class, IUnresolvedMember where TResolved : class, IMember
	{
		...
		for (int i = 0; i < members.unresolvedMembers.Length; i++)
		{
			if (members.unresolvedMembers[i] is TUnresolved obj && (filter == null || filter(obj)))
			{
	...
	public virtual IEnumerable<IMethod> GetMethods(Predicate<IUnresolvedMethod> filter = null, GetMemberOptions options = GetMemberOptions.None)
	{
		...
		{
			return GetFilteredMethods(ExtensionMethods.And((IUnresolvedMethod m) => !m.IsConstructor, filter));
		}
	...
	public virtual IEnumerable<IMethod> GetConstructors(Predicate<IUnresolvedMethod> filter = null, GetMemberOptions options = GetMemberOptions.IgnoreInheritedMembers)
	{
		...
		if ((options & GetMemberOptions.IgnoreInheritedMembers) == GetMemberOptions.IgnoreInheritedMembers)
		{
			return GetFilteredMethods(ExtensionMethods.And((IUnresolvedMethod m) => m.IsConstructor && !m.IsStatic, filter));
		}
	...
	{
		return GetInterfaceImplementation(new IMember[1] { interfaceMember })[0];
	}
