...
public class DefaultUnresolvedAssembly : AbstractFreezable, IUnresolvedAssembly, IAssemblyReference
{
	...
	private sealed class DefaultResolvedAssembly : IAssembly, ICompilationProvider
	{
		private sealed class NS : INamespace, ISymbol, ICompilationProvider
		{
			...
			IEnumerable<IAssembly> INamespace.ContributingAssemblies => new DefaultResolvedAssembly[1] { assembly };
			...
			ITypeDefinition INamespace.GetTypeDefinition(string name, int typeParameterCount)
			{
				...
				if (assembly.unresolvedTypeDict.TryGetValue(key, out var value))
				{
		...
		public IEnumerable<ITypeDefinition> TopLevelTypeDefinitions => unresolvedAssembly.TopLevelTypeDefinitions.Select((IUnresolvedTypeDefinition t) => GetTypeDefinition(t));
		...
		{
			if (unresolvedAssembly.typeDefinitions.TryGetValue(topLevelTypeName, out var value))
			{
				return GetTypeDefinition(value);
			}
			if (unresolvedAssembly.typeForwarders.TryGetValue(topLevelTypeName, out var value2))
			{
	...
	public void AddTypeForwarder(TopLevelTypeName typeName, ITypeReference referencedType)
	{
		...
		FreezableHelper.ThrowIfFrozen(this);
		DefaultUnresolvedAttribute defaultUnresolvedAttribute = new DefaultUnresolvedAttribute(typeForwardedToAttributeTypeRef, new KnownTypeReference[1] { KnownTypeReference.Type });
	...
	public IUnresolvedTypeDefinition GetTypeDefinition(string ns, string name, int typeParameterCount)
	{
		...
		if (typeDefinitions.TryGetValue(key, out var value))
		{
			return value;
		}
		return null;
