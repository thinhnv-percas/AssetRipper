...
public class DefaultResolvedMethod : AbstractResolvedMember, IMethod, IParameterizedMember, IMember, IEntity, ISymbol, ICompilationProvider, INamedElement, IHasAccessibility
{
	...
	public IList<IUnresolvedMethod> Parts => parts ?? new IUnresolvedMethod[1] { (IUnresolvedMethod)unresolved };
	...
	public override ISymbolReference ToReference()
	{
		...
		return new DefaultMemberReference(base.SymbolKind, typeReference2, base.Name, TypeParameters.Count, Parameters.Select((IParameter p) => p.Type.ToTypeReference()).ToList());
	}
	...
	{
		IMethod dummyConstructor = GetDummyConstructor(compilation);
