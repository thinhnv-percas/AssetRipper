...
public sealed class PEReader : IDisposable
{
	...
	{
		Stream stream = GetPEImage().GetStream(out var constraints);
		PEHeaders value;
	...
	internal unsafe static ImmutableArray<byte> DecodeEmbeddedPortablePdbDebugDirectoryData(AbstractMemoryBlock block)
	{
		...
		DeflateStream val = new DeflateStream((Stream)readOnlyUnmanagedMemoryStream, (CompressionMode)0, true);
		if (num > 0)
		{
			int num2;
			try
			{
				num2 = ((Stream)(object)val).TryReadAll(array, 0, array.Length);
			}
