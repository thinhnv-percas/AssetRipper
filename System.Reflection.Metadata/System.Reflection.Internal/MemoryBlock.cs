...
internal readonly struct MemoryBlock
{
	...
	{
		FastComparisonResult fastComparisonResult = Utf8NullTerminatedFastCompare(offset, text, 0, out var _, terminator, ignoreCase);
		if (fastComparisonResult == FastComparisonResult.Inconclusive)
		{
			string text2 = PeekUtf8NullTerminated(offset, null, utf8Decoder, out var _, terminator);
