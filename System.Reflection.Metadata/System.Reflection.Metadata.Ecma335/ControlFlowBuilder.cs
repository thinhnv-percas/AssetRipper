...
public sealed class ControlFlowBuilder
{
	...
	internal void SerializeExceptionTable(BlobBuilder builder)
	{
		...
		foreach (ExceptionHandlerInfo lazyExceptionHandler in _lazyExceptionHandlers)
		{
			...
			exceptionRegionEncoder.AddUnchecked(catchTokenOrOffset: lazyExceptionHandler.Kind switch
			{
				...
				_ => 0, 
			}, kind: lazyExceptionHandler.Kind, tryOffset: labelOffsetChecked, tryLength: labelOffsetChecked2 - labelOffsetChecked, handlerOffset: labelOffsetChecked3, handlerLength: labelOffsetChecked4 - labelOffsetChecked3);
		}
	}
