...
public interface ISymUnmanagedAsyncMethod
{
	...
	[PreserveSig]
	int GetAsyncStepInfo(int bufferLength, out int count, [In][Out][MarshalAs(UnmanagedType.LPArray, SizeParamIndex = 0)] int[] yieldOffsets, [In][Out][MarshalAs(UnmanagedType.LPArray, SizeParamIndex = 0)] int[] breakpointOffset, [In][Out][MarshalAs(UnmanagedType.LPArray, SizeParamIndex = 0)] int[] breakpointMethod);
}
